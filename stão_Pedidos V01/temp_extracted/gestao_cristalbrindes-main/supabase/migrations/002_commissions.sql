...
-- 2. Add 'ENTRE FINALIZADO' to order_status enum
-- Note: This must be run outside of a transaction block in some SQL tools.
-- If it fails, run it separately in the Supabase SQL Editor.
alter type order_status add value if not exists 'ENTRE FINALIZADO';

-- 3. Update save_order function to handle Updates and Commissions
