...
const FinancialDashboardPage: React.FC = () => {
    ...
    const DreRow = ({ label, value, bold, indent, color }: { label: string; value: number; bold?: boolean; indent?: boolean; color?: string }) => (
        <div className={`flex justify-between items-center py-2 ${bold ? 'border-t border-gray-200 pt-3' : ''} ${indent ? 'pl-6' : ''}`}>
            ...
            <span className={`text-sm font-mono ${bold ? 'font-bold' : ''} ${color || (value >= 0 ? 'text-gray-900' : 'text-red-600')}`}>{formatCurrency(value)}</span>
        </div>
    );

    const MarginBadge = ({ label, value }: { label: string; value: number }) => (
        <span className={`text-xs font-bold px-2 py-1 rounded-full ${value >= 30 ? 'bg-green-100 text-green-700' : value >= 15 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}`}>
            ...
            <div className="flex gap-1 bg-white border border-gray-200 p-1 rounded-xl">
                {tabs.map(t => (
                    <button key={t.id} onClick={() => setActiveTab(t.id)}
                        className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-sm font-bold transition-all ${activeTab === t.id ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        {[
                            ...
                            <div key={i} onClick={() => kpi.path && navigate(kpi.path)} className={`bg-white rounded-xl border border-gray-200 p-4 transition-all ${kpi.path ? 'cursor-pointer hover:shadow-lg hover:border-gray-300 hover:-translate-y-0.5' : 'hover:shadow-md'}`}>
                                ...
                                <p className={`text-xs mt-1 ${kpi.sub.includes('vencido') ? 'text-red-500 font-bold' : 'text-gray-400'}`}>{kpi.sub}</p>
                            </div>
                        ))}
                    </div>
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 p-5">
                            ...
                            {cashFlowData.filter(c => c.days <= 30).map(c => (
                                <div key={c.days} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                                    <span className="text-xs font-bold text-gray-500 w-10">{c.period}</span>
                                    ...
                                    <span className={`text-sm font-bold ${c.net >= 0 ? 'text-green-600' : 'text-red-600'}`}>{formatCurrency(c.net)}</span>
                                </div>
                            ))}
                        </div>
                            ...
                            <div className="space-y-4">
                                <div>
                                    <div className="flex justify-between text-xs text-gray-500 mb-1">
                                        <span>Inadimplência</span><span className={kpis.inadimplencia > 20 ? 'text-red-600 font-bold' : ''}>{kpis.inadimplencia.toFixed(1)}%</span>
                                    </div>
                                ...
                                {overdueFlow.overdueIn > 0 && (
                                    <div className="bg-red-50 rounded-lg p-3 mt-2">
                                        <p className="text-xs font-bold text-red-700">⚠ Vencidos a Receber: {formatCurrency(overdueFlow.overdueIn)}</p>
                                    </div>
                                )}
                                {overdueFlow.overdueOut > 0 && (
                                    <div className="bg-orange-50 rounded-lg p-3">
                                        <p className="text-xs font-bold text-orange-700">⚠ Despesas Vencidas: {formatCurrency(overdueFlow.overdueOut)}</p>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            {activeTab === 'cashflow' && (
                <div className="space-y-4">
                    <div className="flex justify-between items-center">
                        ...
                        <button onClick={exportCashFlow} className="flex items-center gap-1 text-xs font-bold text-blue-600 hover:text-blue-800 transition-colors">
                    ...
                    {(overdueFlow.overdueIn > 0 || overdueFlow.overdueOut > 0) && (
                        <div className="bg-red-50 border border-red-200 rounded-xl p-4 grid grid-cols-2 gap-4">
                            <div><span className="text-[10px] font-bold text-red-500 uppercase">Vencido a Receber</span><p className="text-lg font-black text-red-700">{formatCurrency(overdueFlow.overdueIn)}</p></div>
                            <div><span className="text-[10px] font-bold text-orange-500 uppercase">Vencido a Pagar</span><p className="text-lg font-black text-orange-700">{formatCurrency(overdueFlow.overdueOut)}</p></div>
                        </div>
                    )}
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                        <table className="w-full text-sm">
                            <thead><tr className="bg-white text-gray-500 text-[10px] font-bold uppercase border-b border-gray-100">
                            ...
                            <tbody>
                                {cashFlowData.map(c => (
                                    <tr key={c.days} className="border-b border-gray-50 last:border-0 hover:bg-gray-50">
                                        <td className="px-4 py-3 font-bold text-gray-700">{c.period}</td>
                                        ...
                                        <td className={`px-4 py-3 text-right font-bold font-mono ${c.net >= 0 ? 'text-blue-600' : 'text-red-600'}`}>{formatCurrency(c.net)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
            ...
            {activeTab === 'dre' && (
                <div className="space-y-4">
                    <div className="flex justify-between items-center flex-wrap gap-3">
                        ...
                        <button onClick={exportDRE} className="flex items-center gap-1 text-xs font-bold text-blue-600 hover:text-blue-800">
                            <span className="material-icons-outlined text-sm">download</span> CSV
                        </button>
                    </div>

                    <div className="bg-white rounded-xl border border-gray-200 p-5">
                        <p className="text-xs font-bold text-gray-400 uppercase mb-2">Demonstrativo — {dreData.totalPedidos} pedidos no mês</p>
