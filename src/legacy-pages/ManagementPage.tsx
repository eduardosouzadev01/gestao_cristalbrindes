...
const ManagementPage: React.FC = () => {
    ...
    const deleteLead = async (id: string, showConfirm = true) => {
        if (showConfirm && !window.confirm('Excluir este atendimento?')) return;
    ...
    return (
        <div className="w-full">
            <div className="max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                    <div>
                        <h1 className="text-2xl font-medium text-gray-900 uppercase tracking-tight flex items-center gap-2">
                ...
                <div className="flex items-center gap-1 bg-transparent">
                    <button
                        ...
                        className="px-4 py-2 bg-[#1565C0] text-white rounded-md text-[11px] font-medium uppercase tracking-widest shadow-none shadow-none-blue-500/20 hover:bg-blue-700 hover:shadow-none-blue-500/40 transition-all flex items-center gap-2"
                    >
                    ...
                    {(!isSeller || hasPermission('crm.performance')) && (
                        <button
                            ...
                            className={`px-4 py-2 rounded-md text-[11px] font-medium uppercase transition-all flex items-center gap-2 ${activeTab === 'TRANSFERENCIAS' ? 'bg-[#1565C0] text-white' : 'bg-transparent text-gray-500 hover:text-[#1565C0] hover:underline decoration-2 underline-offset-4'}`}
                        >
                            <span className="material-icons-outlined text-sm">swap_horiz</span>
                            Trocas
                            {pendingTransferCount > 0 && (
                                <span className="flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[10px] font-medium text-white shadow-none animate-pulse">
                    ...
                    <button
                        ...
                        className={`px-4 py-2 rounded-md text-[11px] font-medium uppercase transition-all flex items-center gap-2 ${activeTab === 'RECORDS' ? 'bg-[#1565C0] text-white' : 'bg-transparent text-gray-400 hover:text-[#1565C0] hover:underline decoration-2 underline-offset-4'}`}
                    >
            ...
            {activeTab === 'RECORDS' && (
                <div className="animate-in fade-in slide-in-from-top-4 duration-500">
                            ...
                            return (
                                <div 
                                    ...
                                    className={`flex-1 min-w-[200px] p-5 rounded-md border flex items-center gap-4 transition-all group cursor-pointer ${
                                        isActive 
                                        ? 'bg-blue-600 border-blue-700 shadow-none shadow-none-blue-200 ring-4 ring-blue-500/10' 
                                        : 'bg-white border-gray-100 shadow-none hover:shadow-none hover:border-blue-100'
                                    }`}
                                >
                                    ...
                                    <div>
                                        <p className={`text-[10px] uppercase font-medium tracking-widest ${isActive ? 'text-blue-100' : 'text-gray-400'}`}>{seller}</p>
                    ...
                    <div className="bg-white rounded-md border border-gray-100 shadow-none-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
                        <div className="p-6 border-b border-gray-100 bg-white">
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-6">
                                <div>
                                    <h2 className="text-lg font-medium text-gray-900 uppercase tracking-tight">Planilha de Atendimentos</h2>
                                    <p className="text-[10px] text-gray-400 font-medium uppercase tracking-widest mt-0.5">Histórico e gestão de leads ativos e finalizados</p>
                                </div>
                                <div className="flex items-center gap-2">
                                    <button onClick={() => fetchLeads()} className="p-2.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-all" title="Sincronizar dados">
                            ...
                            <div className="flex flex-wrap items-center gap-3">
                                <div className="relative flex-1 min-w-[280px]">
                                    ...
                                    <input 
                                        ...
                                        className="w-full pl-14 pr-4 py-2.5 bg-gray-50/50 border border-gray-100 rounded-md text-[11px] font-medium text-gray-600 uppercase tracking-tight outline-none focus:ring-4 focus:ring-blue-500/10 focus:bg-white focus:border-blue-200 transition-all"
                                        value={searchTerm}
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <select 
                                            className="pl-9 pr-8 py-2.5 bg-white border border-gray-100 rounded-md text-[11px] font-medium text-gray-600 uppercase tracking-tight outline-none focus:ring-4 focus:ring-blue-500/10 hover:bg-gray-50 transition-all appearance-none"
                                    ...
                                    {/* Seller Filter */}
                                    <div className="relative">
                                        <span className="material-icons-outlined absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">person</span>
                                        <select 
                                            className="pl-9 pr-8 py-2.5 bg-white border border-gray-100 rounded-md text-[11px] font-medium text-gray-600 uppercase tracking-tight outline-none focus:ring-4 focus:ring-blue-500/10 hover:bg-gray-50 transition-all appearance-none"
                                    ...
                                    {/* Period Filter */}
                                    <div className="relative">
                                        <span className="material-icons-outlined absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">history</span>
                                        <select
                                            className="pl-9 pr-8 py-2.5 bg-white border border-gray-100 rounded-md text-[11px] font-medium text-gray-600 uppercase tracking-tight outline-none focus:ring-4 focus:ring-blue-500/10 hover:bg-gray-50 transition-all appearance-none"
                                            value={kanbanPeriodFilter}
                                            onChange={e => {
                                                const val = e.target.value;
                                                setKanbanPeriodFilter(val);
                                    ...
                                    {/* Date inputs only show if Custom is selected */}
                                    {kanbanPeriodFilter === 'Custom' && (
                                        <div className="flex items-center gap-2 bg-blue-50/50 p-1.5 rounded-md border border-blue-100 animate-in fade-in zoom-in-95 duration-200">
                                            <input
                                                type="date"
                                                className="border-0 bg-transparent text-[10px] font-medium text-blue-700 uppercase focus:ring-0 p-0 w-[100px]"
                                                value={startDate}
                                                onChange={e => setStartDate(e.target.value)}
                                            />
                                            <span className="text-blue-300 text-[10px] font-medium">/</span>
                                            <input
                                                type="date"
                                                className="border-0 bg-transparent text-[10px] font-medium text-blue-700 uppercase focus:ring-0 p-0 w-[100px]"
                                                value={endDate}
                            ...
                            <table className="min-w-full divide-y divide-gray-300 text-[14px]">
                                <thead>
                                    <tr className="bg-gray-100/50">
                                        <th className="px-6 py-4 text-left text-[11px] font-medium text-gray-500 uppercase tracking-widest border-b border-gray-200">Atendimento</th>
                                        <th className="px-6 py-4 text-left text-[11px] font-medium text-gray-500 uppercase tracking-widest border-b border-gray-200">Prioridade</th>
                                        <th className="px-6 py-4 text-left text-[11px] font-medium text-gray-500 uppercase tracking-widest border-b border-gray-200">Data</th>
                                        <th className="px-6 py-4 text-left text-[11px] font-medium text-gray-500 uppercase tracking-widest border-b border-gray-200">Cliente / Empresa</th>
                                        <th className="px-6 py-4 text-left text-[11px] font-medium text-gray-500 uppercase tracking-widest border-b border-gray-200">Item / Projeto</th>
                                        <th className="px-6 py-4 text-left text-[11px] font-medium text-gray-500 uppercase tracking-widest border-b border-gray-200">Vendedor</th>
                                        <th className="px-6 py-4 text-right text-[11px] font-medium text-gray-500 uppercase tracking-widest border-b border-gray-200">Ações Rápidas</th>
                                    </tr>
                                </thead>
                                <tbody className="bg-white divide-y divide-gray-100">
                                    {loading ? (
                                        <tr><td colSpan={8} className="px-6 py-10 text-center text-gray-400 font-medium uppercase text-xs animate-pulse tracking-widest">Carregando dados da nuvem...</td></tr>
                                    ) : (leads || [])
                                        ...
                                        .map(l => (
                                            <LeadTableRow
                                                ...
                                                onTransfer={(lead) => {
                                                    setLeadToDirectTransfer(lead);
                                                    setTargetSalesperson(lead.salesperson || '');
                                                    setIsDirectTransferModalOpen(true);
                                                }}
                                                onConvertToBudget={convertToBudget}
                                            />
                                        ))}
                                </tbody>
                            </table>
...

            {
                activeTab === 'TRANSFERENCIAS' && (
                    <div className="bg-white rounded-md border border-gray-200 shadow-none overflow-hidden animate-in fade-in duration-300">
                        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                            <div>
                                <h2 className="text-lg font-medium text-gray-900 uppercase">Solicitações de Troca de Carteira</h2>
                                <p className="text-xs text-gray-500 font-medium uppercase tracking-widest">Aprovação de transferências entre vendedores</p>
                            </div>
                            <button onClick={fetchTransferRequests} className="p-2 text-blue-600 hover:bg-blue-50 rounded-md transition-all">
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead>
                                    <tr className="bg-gray-50/80">
                                        <th className="px-6 py-3 text-left text-[10px] font-medium text-gray-400 uppercase tracking-widest">Tipo</th>
                                        <th className="px-6 py-3 text-left text-[10px] font-medium text-gray-400 uppercase tracking-widest">Alvo</th>
                                        <th className="px-6 py-3 text-left text-[10px] font-medium text-gray-400 uppercase tracking-widest">Origem</th>
                                        <th className="px-6 py-3 text-left text-[10px] font-medium text-gray-400 uppercase tracking-widest">Destino</th>
                                        <th className="px-6 py-3 text-left text-[10px] font-medium text-gray-400 uppercase tracking-widest">Data</th>
                                        <th className="px-6 py-3 text-left text-[10px] font-medium text-gray-400 uppercase tracking-widest">Status</th>
                                        <th className="px-6 py-3 text-right text-[10px] font-medium text-gray-400 uppercase tracking-widest">Ações</th>
                                    </tr>
                                </thead>
                                <tbody className="bg-white divide-y divide-gray-100">
                                    {loadingTransfers ? (
                                        <tr><td colSpan={7} className="px-6 py-10 text-center text-gray-400 font-medium uppercase text-xs">Carregando...</td></tr>
                                    ) : transferRequests.length === 0 ? (
                                        <tr><td colSpan={7} className="px-6 py-10 text-center text-gray-400 font-medium uppercase text-xs">Nenhuma solicitação encontrada.</td></tr>
                                    ) : (
                                        transferRequests.map(req => (
                                            <tr key={req.id} className="hover:bg-gray-50/50 transition-colors">
                                                <td className="px-6 py-4">
                                                    <span className={`px-2 py-0.5 rounded-md text-[8px] font-medium uppercase tracking-tighter ${req.type === 'CLIENT' ? 'bg-indigo-100 text-indigo-700' : 'bg-amber-100 text-amber-700'}`}>
                                                        {req.type === 'CLIENT' ? 'Carteira' : 'Pedido'}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <p className="text-sm font-medium text-gray-900">{req.type === 'CLIENT' ? fixClientName(req.client?.name || '') : `Pedido #${req.order_number}`}</p>
                                                    <p className="text-[10px] text-gray-400 font-medium italic">"{req.reason || 'Sem motivo informado'}"</p>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <span className="text-xs font-medium text-gray-600">{req.current_salesperson}</span>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <span className="text-xs font-medium text-blue-600">{req.type === 'CLIENT' ? req.requesting_salesperson : req.requested_salesperson}</span>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <span className="text-xs text-gray-500">{new Date(req.created_at).toLocaleDateString()}</span>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <span className={`px-2 py-1 rounded-md text-[9px] font-medium uppercase tracking-widest ${req.status === 'APPROVED' ? 'bg-green-100 text-green-700' :
                                                ...
                                                <td className="px-6 py-4 text-right space-x-2">
                                                    {req.status === 'PENDING' && (
                                                        <>
                                                            <button onClick={() => handleApproveTransfer(req)} className="px-3 py-1.5 bg-green-500 text-white text-[9px] font-medium uppercase tracking-widest rounded-md hover:bg-green-600 transition-all shadow-none">Aprovar</button>
                                                            <button onClick={() => handleRejectTransfer(req)} className="px-3 py-1.5 bg-red-500 text-white text-[9px] font-medium uppercase tracking-widest rounded-md hover:bg-red-600 transition-all shadow-none">Rejeitar</button>
                                                        </>
                                                    )}
                                                </td>
                                            </tr>
            ...
            <LeadModal
                ...
                onClose={() => { setIsLeadModalOpen(false); setEditingLead(null); setNewLead(initialLeadState); }}
                ...
                saveLead={saveLead}
                appUser={appUser}
                userSalesperson={userSalesperson}
                isSeller={isSeller}
                        ...
                        <div className="bg-white rounded-md w-full max-w-sm shadow-none overflow-hidden animate-in zoom-in-95 duration-200 border border-red-100">
                            ...
                            <div className="p-8 space-y-6">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        {['PREÇO', 'PRAZO', 'CONCORRÊNCIA', 'OUTRO'].map(cat => (
                                            <button
                                                ...
                                                className={`py-2 rounded-md text-[10px] font-medium uppercase transition-all border ${lostForm.category === cat ? 'bg-red-500 text-white border-red-500 shadow-none scale-105' : 'bg-white text-gray-400 border-gray-100 hover:border-red-200'}`}
                                            >
                                                {cat}
                                            </button>
                                        ))}
                                ...
                                <div>
                                    ...
                                    <textarea
                                        className="form-textarea w-full rounded-md border-gray-100 focus:ring-red-500 focus:border-red-500 text-sm font-medium bg-gray-50"
                                        rows={3}
                        ...
                        <div className="bg-white rounded-md w-full max-w-sm shadow-none overflow-hidden animate-in zoom-in-95 duration-200 border border-green-100">
                            ...
                            <div className="p-8 space-y-6">
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <button
                                            ...
                                            className={`py-3 rounded-md text-xs font-medium uppercase transition-all border ${finalizeForm.success ? 'bg-green-600 text-white border-green-600 shadow-none transform scale-105' : 'bg-gray-50 text-gray-400 border-gray-100 hover:bg-gray-100'}`}
                                        >
                                            Sim, Fez Pedido
                                        </button>
                                        <button
                                            onClick={() => setFinalizeForm({ ...finalizeForm, success: false, category: '' })}
                                            className={`py-3 rounded-md text-xs font-medium uppercase transition-all border ${!finalizeForm.success ? 'bg-red-600 text-white border-red-600 shadow-none transform scale-105' : 'bg-gray-50 text-gray-400 border-gray-100 hover:bg-gray-100'}`}
                                        >
                                            Não Fez
                                        </button>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        {(finalizeForm.success 
                                            ...
                                            <button
                                                ...
                                                className={`py-2 px-3 rounded-md border text-[10px] font-medium uppercase transition-all ${finalizeForm.category === cat ? 'bg-blue-600 border-blue-600 text-white shadow-none' : 'bg-white border-gray-100 text-gray-400 hover:border-blue-300 hover:bg-blue-50'}`}
                                            >
                                                {cat}
                                            </button>
                                        ))}
                                ...
                                <div>
                                    ...
                                    <textarea
                                        className="form-textarea w-full rounded-md border-gray-100 focus:ring-blue-500 focus:border-blue-500 text-sm font-medium bg-gray-50"
                                        rows={3}
            ...
            {/* Check Client Modal */}
            {
                isCheckClientModalOpen && (
                    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
                        <div className="bg-white rounded-md w-full max-w-sm shadow-none overflow-hidden animate-in zoom-in-95 duration-200">
                            <div className="p-8 text-center bg-gray-50 border-b border-gray-100">
                                <h3 className="text-xl font-medium text-gray-900 uppercase">Novo Atendimento</h3>
                                <p className="text-xs text-gray-500 mt-2 font-medium uppercase tracking-widest">Verificação de Cliente</p>
                            </div>
                            <div className="p-8 space-y-6">
                                <div>
                                    <label className="block text-[10px] font-medium text-gray-400 uppercase mb-2 tracking-widest">E-mail ou Telefone</label>
                                    <input
                                        type="text"
                                        className="form-input w-full rounded-md border-gray-200 focus:ring-blue-500 focus:border-blue-500 text-lg font-medium text-center py-3"
                                ...
                                <button
                                    ...
                                    className="w-full py-4 bg-blue-600 text-white rounded-md font-medium uppercase text-xs shadow-none hover:bg-blue-700 transition-all active:scale-95 flex items-center justify-center gap-2"
                                >
                                ...
                                <button
                                    ...
                                    className="w-full py-2 text-gray-400 font-medium uppercase text-[10px] hover:text-gray-600"
                                >
                                    Cancelar
                                </button>
                            </div>
            ...
            {/* Request Transfer Modal (Small/Elegant) */}
            {
                isTransferModalOpen && (
                    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                        <div className="bg-white rounded-md w-full max-w-md shadow-none overflow-hidden animate-in slide-in-from-bottom-5 duration-300">
                            <div className="p-6 bg-orange-50 border-b border-orange-100 flex items-center gap-4">
                                <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-none-inner text-orange-500">
                                    <span className="material-icons-outlined text-2xl">swap_horiz</span>
                                </div>
                                <div>
                                    <h3 className="text-sm font-medium text-gray-900 uppercase">Solicitar Troca de Vendedor</h3>
                                    <p className="text-[10px] text-orange-600 font-medium uppercase tracking-widest">Este cliente pertence a {selectedClientToTransfer?.salesperson}</p>
                                </div>
                            </div>

                            <div className="p-6 space-y-4">
                                <div className="bg-gray-50 rounded-md p-4 border border-gray-100">
                                    <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest mb-1">CLIENTE SELECIONADO</p>
                                    <p className="text-sm font-medium text-gray-800">{selectedClientToTransfer?.name}</p>
                                    <p className="text-xs text-gray-500">{selectedClientToTransfer?.email || 'N/A'} | {selectedClientToTransfer?.phone || 'N/A'}</p>
                                </div>

                                <div className="bg-blue-50 rounded-md p-4 border border-blue-100 flex items-start gap-3">
                                ...
                                <div className="flex gap-4 pt-4">
                                    <button
                                        ...
                                        className="flex-1 py-3 text-gray-400 font-medium uppercase text-[10px] hover:text-gray-600 transition-colors"
                                    >
                                        Cancelar
                                    </button>
                                    <button
                                        onClick={handleRequestTransfer}
                                        className="flex-1 py-4 rounded-md font-medium uppercase text-[10px] shadow-none transition-all active:scale-95 bg-orange-500 text-white hover:bg-orange-600 shadow-none-orange-200"
                                    >
                                        Enviar Solicitação
                                    </button>
                                </div>
                            ...
                            <div className="p-8 pb-4 flex flex-col md:flex-row items-center gap-6">
                                ...
                                <div className="text-center md:text-left flex-1">
                                    ...
                                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-50/50 rounded-full border border-blue-100">
                                        ...
                                        <span className="text-[10px] text-blue-600 font-medium uppercase tracking-widest">{leadToDirectTransfer?.client_name}</span>
                                    </div>
                                </div>
                            ...
                            <div className="px-8 pb-10 pt-2 space-y-8">
                                        ...
                                        {SALESPERSON_OPTIONS.map(sp => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`group relative p-4 rounded-md border-2 transition-all duration-300 flex flex-col items-center gap-3 text-center ${isSelected ? 'bg-blue-600 border-blue-600 text-white shadow-none shadow-none-blue-200 scale-105 z-10' : 'bg-slate-50/50 border-transparent text-slate-400 hover:border-blue-200 hover:bg-white hover:shadow-none'}`}
                                                >
                                                    <div className={`w-12 h-12 rounded-md flex items-center justify-center text-sm font-medium transition-all ${isSelected ? 'bg-white/20' : 'bg-white shadow-none group-hover:bg-blue-50 group-hover:text-blue-600'}`}>
                                                        {getSalespersonInitials(sp)}
                                                    </div>
                                                    <div>
                                                        <p className={`text-[10px] font-medium uppercase tracking-wider leading-none ${isSelected ? 'text-white' : 'text-slate-800'}`}>{sp.replace('VENDAS ', 'V')}</p>
                                                        <p className={`text-[8px] font-medium uppercase mt-1 tracking-tighter ${isSelected ? 'text-white/70' : 'text-slate-400'}`}>Equipe</p>
                                                    </div>
                                                    {isSelected && (
                                ...
                                <div className="flex flex-col md:flex-row items-center gap-4">
                                    <button
                                        ...
                                        className={`flex-1 w-full py-5 rounded-md font-medium uppercase text-[11px] tracking-[0.15em] transition-all active:scale-95 flex items-center justify-center gap-3 ${!targetSalesperson || isTransferring || targetSalesperson === leadToDirectTransfer?.salesperson ? 'bg-slate-100 text-slate-300 cursor-not-allowed' : 'bg-slate-900 text-white hover:bg-blue-600 shadow-none shadow-none-slate-200 hover:shadow-none-blue-200'}`}
                                    >
                                        {isTransferring ? (
                                            <>
