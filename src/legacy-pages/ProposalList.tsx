        ...
        <div className="max-w-[1920px] w-full mx-auto px-4 py-4 space-y-4">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-xl font-medium text-gray-900 uppercase tracking-tighter">Propostas Comerciais</h2>
                </div>
            </div>

            <div className="bg-white rounded-md border border-gray-200 shadow-none overflow-hidden border-t-2 border-t-blue-600">
                <div className="px-3 py-2 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
                    <div className="relative w-full max-w-sm">
                        ...
                        <input
                            ...
                            className="w-full !pl-10 pr-3 py-1 text-xs border-gray-300 rounded-md focus:ring-1 focus:ring-blue-500 focus:border-blue-500 h-8"
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                        />
                    </div>
                    <div className="flex items-center gap-4 text-[10px] font-medium text-gray-400 uppercase tracking-widest pl-4 border-l border-gray-200 ml-4 hidden md:flex">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead className="bg-gray-50/50 text-[9px] font-medium text-gray-400 uppercase tracking-widest border-b border-gray-100">
                            <tr>
                        ...
                        <tbody className="divide-y divide-gray-100 bg-white">
                            {filteredProposals.length > 0 ? (
                                filteredProposals.map((item) => (
                                    <tr key={item.id} className="hover:bg-blue-50/50 transition-colors group cursor-pointer" onClick={() => navigate(`/proposta/${item.id}`)}>
                                        <td className="px-3 py-1.5 whitespace-nowrap">
                                            <span className="text-xs font-medium text-blue-600">#{item.proposal_number}</span>
                                        </td>
                                        <td className="px-3 py-1.5 whitespace-nowrap">
                                            <span className="text-xs font-medium text-gray-800 uppercase leading-none">{fixClientName(item.client?.name || 'Vários')}</span>
                                        </td>
                                        <td className="px-3 py-1.5 text-center whitespace-nowrap">
                                            <span className="text-gray-500 text-[10px] font-medium uppercase">{new Date(item.created_at).toLocaleDateString('pt-BR')}</span>
                                        </td>
                                        <td className="px-3 py-1.5 text-center whitespace-nowrap">
                                            <span className="text-gray-400 text-[10px] font-medium uppercase leading-none">{item.salesperson}</span>
                                        </td>
                                        <td className="px-3 py-1.5 text-right whitespace-nowrap">
                                            <span className="text-xs font-medium text-gray-900">{formatCurrency(item.total_amount)}</span>
                                        </td>
                                        <td className="px-3 py-1.5 text-center whitespace-nowrap">
                                            <span className={`px-2 py-0.5 rounded-md text-[8px] font-medium uppercase shadow-none ${item.status === 'GERADA' ? 'bg-blue-500 text-white' :
                                        ...
                                        <td className="px-3 py-1.5 text-right whitespace-nowrap">
                                            <div className="flex justify-end gap-2 pr-1">
                                                <button
                                                    ...
                                                    className="p-1 text-gray-400 hover:text-blue-600 hover:bg-white hover:shadow-none rounded-md transition-all"
                                                    title="Ver Proposta"
                                                >
                                                ...
                                                {(appUser?.permissions?.fullAccess || appUser?.permissions?.canDelete) && (
                                                    <button
                                                        ...
                                                        className="p-1 text-gray-400 hover:text-red-500 hover:bg-white hover:shadow-none rounded-md transition-all"
                                ...
                                <tr>
                                    <td colSpan={7} className="px-3 py-10 text-center text-gray-400 italic text-[10px] font-medium uppercase">
