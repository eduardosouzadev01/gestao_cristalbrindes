    ...
    <div className="max-w-[1920px] w-full mx-auto px-4 py-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-red-600 rounded-md flex items-center justify-center shadow-none">
            <span className="material-icons-outlined text-white text-lg">payments</span>
          </div>
          <div>
            <h1 className="text-xl font-medium leading-none text-gray-900 uppercase tracking-tighter">CONTAS A PAGAR</h1>
            <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest mt-0.5">Fluxo de custos de pedidos e despesas operacionais</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={exportData} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-gray-300 text-[10px] font-medium uppercase tracking-widest text-gray-700 rounded-md shadow-none hover:bg-gray-50 transition-all cursor-pointer">
            <span className="material-icons-outlined text-sm">download</span> CSV
          </button>
          {activeTab === 'expenses' && (
            <button onClick={() => setExpenseModal(true)} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white text-[10px] font-medium uppercase tracking-widest rounded-md shadow-none hover:bg-blue-700 transition-all cursor-pointer">
      ...
      <div className="bg-white rounded-md border border-gray-200 overflow-hidden shadow-none">
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-medium text-[11px] text-gray-700 uppercase tracking-widest">Resumo Financeiro Gerencial</span>
            {stats.overdue > 0 && (
              <span className="px-1.5 py-0.5 rounded-md bg-red-50 text-red-600 text-[8px] font-medium uppercase border border-red-100 animate-pulse">
        ...
        {showSummary && (
          <div className="p-3 border-t border-gray-100 bg-white grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-2 anime-in slide-in-from-top-2">
            {[
              ...
              <div key={div.label} className={`bg-white rounded-md border p-2.5 shadow-none relative group overflow-hidden ${div.label === 'VENCIDO' && stats.overdue > 0 ? 'border-red-200 ring-1 ring-red-50' : 'border-gray-200'}`}>
                <div className={`absolute top-0 left-0 w-0.5 h-full bg-${div.color}-500/50`}></div>
                <div className="flex items-center justify-between mb-1">
                  <span className={`text-[7px] font-medium uppercase tracking-widest ${div.label === 'VENCIDO' && stats.overdue > 0 ? 'text-red-500' : 'text-gray-400'}`}>{div.label}</span>
                  <span className={`material-icons-outlined text-[10px] text-${div.color}-500`}>{div.icon}</span>
                </div>
                <p className={`text-base font-medium text-gray-900 leading-none group-hover:scale-105 transition-transform`}>{formatCurrency(div.value)}</p>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="grid grid-cols-12 gap-3 items-end">
        <div className="col-span-12 lg:col-span-4 bg-white p-1 rounded-md border border-gray-200 flex gap-1">
          {[
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 px-2 rounded-md text-[9px] font-medium uppercase tracking-widest transition-all ${activeTab === t.id ? 'bg-white text-blue-600 shadow-none border border-gray-100' : 'text-gray-400 hover:text-gray-600'}`}
            >
        ...
        <div className="col-span-12 lg:col-span-8 bg-white p-1.5 rounded-md border border-gray-200 shadow-none flex items-center flex-wrap gap-2">
          <div className="flex-1 min-w-[150px] relative">
            <span className="material-icons-outlined absolute left-2 top-1/2 -translate-y-1/2 text-gray-300 text-sm">search</span>
            <input
              className="w-full !pl-10 h-8 border border-gray-300 rounded-md text-[10px] font-medium uppercase placeholder:text-gray-300"
              placeholder="BUSCAR REGISTRO..."
              value={searchTerm}
              onChange={e => setSearchTerm(e.target.value)}
            />
          </div>
          <IssuerSelect className="h-8 px-2 border border-gray-300 rounded-md text-[10px] font-medium uppercase bg-white" value={issuerFilter} onChange={setIssuerFilter} includeAll allLabel="TODAS EMPRESAS" />
          <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-md px-1 h-8">
            <input type="date" className="bg-transparent border-0 text-[10px] font-medium p-0 w-24 h-full" value={filterDateFrom} onChange={e => setFilterDateFrom(e.target.value)} />
            <span className="text-gray-300 text-[10px]">→</span>
            <input type="date" className="bg-transparent border-0 text-[10px] font-medium p-0 w-24 h-full" value={filterDateTo} onChange={e => setFilterDateTo(e.target.value)} />
          </div>
          <label className="flex items-center gap-2 px-2 h-8 bg-white border border-gray-200 rounded-md cursor-pointer select-none">
            <input type="checkbox" checked={sortByDate} onChange={e => setSortByDate(e.target.checked)} className="w-3.5 h-3.5 rounded-md border-gray-300 text-blue-600 focus:ring-0" />
            <span className="text-[9px] font-medium text-gray-500 uppercase tracking-tight">CRONOLÓGICO</span>
          </label>
          <label className="flex items-center gap-2 px-2 h-8 bg-white border border-gray-200 rounded-md cursor-pointer select-none">
            <input type="checkbox" checked={showPaid} onChange={e => setShowPaid(e.target.checked)} className="w-3.5 h-3.5 rounded-md border-gray-300 text-blue-600 focus:ring-0" />
            <span className="text-[9px] font-medium text-gray-500 uppercase tracking-tight">EXIBIR PAGOS</span>
          </label>
        </div>
      </div>
        ...
        <div className="bg-white rounded-md border border-gray-200 shadow-none overflow-hidden">
          <div className="overflow-x-auto">
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {filteredCosts.map((c, i) => (
                  <tr
                    ...
                    className={`hover:bg-blue-50/30 transition-colors group ${c.isPaid ? 'bg-white opacity-60' : !c.isPaid && c.dueDate && c.dueDate < todayStr ? 'bg-red-50/50' : ''}`}
                  >
                    <td className="px-4 py-1.5">
                      <div className="flex flex-col min-w-0">
                        <span className="text-[10px] font-medium text-blue-600 hover:text-blue-800 underline underline-offset-2 decoration-blue-200" onClick={() => navigate('/pedido/' + c.orderId)}>
                          #{c.orderNumber}
                        </span>
                        <span className="text-[8px] font-medium text-gray-400 uppercase tracking-tighter truncate max-w-[200px] mt-0.5">{c.productName}</span>
                      </div>
                    </td>
                    <td className="px-4 py-1.5">
                      <span className="px-1.5 py-0.5 rounded-md bg-white text-[8px] font-medium text-gray-600 uppercase border border-gray-200 shadow-none">{c.label}</span>
                    </td>
                    <td className="px-4 py-1.5 whitespace-nowrap">
                      <span className="text-[10px] font-medium text-gray-700 uppercase">{c.supplierName}</span>
                    </td>
                    <td className="px-4 py-1.5 whitespace-nowrap text-center">
                      <span className={`text-[9px] font-medium ${!c.isPaid && c.dueDate && c.dueDate < todayStr ? 'text-red-500 font-medium' : 'text-gray-500'}`}>
                        {c.dueDate ? formatDate(c.dueDate) : '---'}
                      </span>
                    </td>
                    <td className="px-4 py-1.5 whitespace-nowrap text-right">
                      <span className="text-[10px] font-medium text-gray-400 tabular-nums">{formatCurrency(c.estimatedValue)}</span>
                    </td>
                    <td className="px-4 py-1.5 whitespace-nowrap text-right">
                      <span className="text-[10px] font-medium text-gray-900 tabular-nums">{formatCurrency(c.realValue || c.estimatedValue)}</span>
                    </td>
                    <td className="px-4 py-1.5 whitespace-nowrap text-center">
                      <button
                        onClick={(e) => { e.stopPropagation(); setConfirmModal({ isOpen: true, type: 'order', item: c, action: c.isPaid ? 'revert' : 'pay' }); }}
                        className={`inline-flex items-center justify-center min-w-[80px] py-1 px-2 rounded-md text-[8px] font-medium uppercase tracking-widest shadow-none border transition-all ${c.isPaid ? 'bg-emerald-50 text-emerald-600 border-emerald-100 hover:bg-emerald-100' : 'bg-orange-50 text-orange-600 border-orange-100 hover:bg-orange-100'}`}
                      >
                        {c.isPaid ? (
            ...
            {filteredCosts.length === 0 && (
              <div className="py-12 text-center text-[10px] font-medium text-gray-300 uppercase italic">Nenhum custo pendente localizado</div>
            )}
          </div>
        </div>
      )}
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {supplierGroups.map((g, i) => (
                  <tr key={i} className="hover:bg-blue-50/30 transition-colors">
                    <td className="px-4 py-2 text-[10px] font-medium text-gray-700 uppercase">{g.name}</td>
                    <td className="px-4 py-2 text-[10px] font-medium text-gray-500 text-right tabular-nums">{formatCurrency(g.total)}</td>
                    <td className="px-4 py-2 text-[10px] font-medium text-emerald-600 text-right tabular-nums">{formatCurrency(g.paid)}</td>
                    <td className="px-4 py-2 text-[10px] font-medium text-orange-600 text-right tabular-nums">{formatCurrency(g.total - g.paid)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            {supplierGroups.length === 0 && (
              <div className="py-12 text-center text-[10px] font-medium text-gray-300 uppercase italic">Nenhum fornecedor localizado</div>
            )}
          </div>
        </div>
      )}
      ...
      {activeTab === 'expenses' && (
        <div className="bg-white rounded-md border border-gray-200 shadow-none overflow-hidden text-[10px]">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-white">
              <tr className="text-[8px] font-medium text-gray-400 uppercase tracking-widest">
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {filteredExpenses.map(e => (
                <tr key={e.id} className={`hover:bg-blue-50/30 group ${e.paid ? 'opacity-60 bg-white' : e.due_date < todayStr ? 'bg-red-50' : ''}`}>
                  <td className="px-4 py-2 uppercase font-medium text-gray-800">{e.description}</td>
                  <td className="px-4 py-2"><span className="px-1.5 py-0.5 rounded-md bg-white border text-[8px] font-medium uppercase tracking-tighter">{e.category}</span></td>
                  <td className="px-4 py-2 text-gray-400 font-medium uppercase">{e.issuer}</td>
                  <td className="px-4 py-2 text-right font-medium tabular-nums">{formatCurrency(e.amount)}</td>
                  <td className={`px-4 py-2 text-center font-medium ${!e.paid && e.due_date < todayStr ? 'text-red-500 font-medium' : 'text-gray-500'}`}>{formatDate(e.due_date)}</td>
                  <td className="px-4 py-2 text-center">
                    <button onClick={() => setConfirmModal({ isOpen: true, type: 'expense', item: e, action: e.paid ? 'revert' : 'pay' })} className={`inline-flex items-center gap-1 px-3 py-1 rounded-md text-[8px] font-medium uppercase border tracking-widest shadow-none ${e.paid ? 'bg-emerald-50 text-emerald-600 border-emerald-100 font-medium' : 'bg-orange-50 text-orange-600 border-orange-100'}`}>
          ...
          {filteredExpenses.length === 0 && <div className="py-12 text-center text-[10px] font-medium text-gray-300 uppercase italic">Nenhuma despesa localizada</div>}
        </div>
      )}
      ...
      {(confirmModal.isOpen || expenseModal) && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-[2px]">
          {expenseModal ? (
            <div className="bg-white rounded-md border w-full max-w-sm shadow-none animate-in zoom-in-95 overflow-hidden">
              <div className="bg-white px-4 py-3 border-b flex justify-between items-center">
                <h3 className="text-[10px] font-medium uppercase tracking-widest">LANÇAR NOVA DESPESA</h3>
              ...
              <div className="p-4 space-y-3">
                <div>
                  <label className="block text-[8px] font-medium text-gray-400 uppercase mb-1">DESCRIÇÃO DA CONTA</label>
                  <input className="w-full px-2 border border-gray-300 rounded-md h-8 text-xs font-medium uppercase placeholder:text-gray-200" placeholder="EX: ALUGUEL" value={expenseForm.description} onChange={e => setExpenseForm({ ...expenseForm, description: e.target.value })} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[8px] font-medium text-gray-400 uppercase mb-1">VALOR (R$)</label>
                    <input type="number" step="0.01" className="w-full px-2 border border-gray-300 rounded-md h-8 text-xs font-medium" value={expenseForm.amount} onChange={e => setExpenseForm({ ...expenseForm, amount: parseFloat(e.target.value) || 0 })} />
                  </div>
                  <div>
                    <label className="block text-[8px] font-medium text-gray-400 uppercase mb-1">VENCIMENTO</label>
                    <input type="date" className="w-full px-2 border border-gray-300 rounded-md h-8 text-[10px] font-medium" value={expenseForm.due_date} onChange={e => setExpenseForm({ ...expenseForm, due_date: e.target.value })} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[8px] font-medium text-gray-400 uppercase mb-1">CATEGORIA</label>
                    <select className="w-full px-2 border border-gray-300 rounded-md h-8 text-[9px] font-medium uppercase" value={expenseForm.category} onChange={e => setExpenseForm({ ...expenseForm, category: e.target.value })}>
                  ...
                  <div>
                    <label className="block text-[8px] font-medium text-gray-400 uppercase mb-1">EMISSOR</label>
                    <IssuerSelect className="w-full px-2 border border-gray-300 rounded-md h-8 text-[9px] font-medium uppercase" value={expenseForm.issuer} onChange={v => setExpenseForm({ ...expenseForm, issuer: v })} />
                  </div>
                </div>
                <div>
                  <label className="block text-[8px] font-medium text-gray-400 uppercase mb-1 font-medium">RECORRÊNCIA (MESES)</label>
                  <input type="number" min="1" max="120" className="w-full px-2 border border-gray-300 rounded-md h-8 text-xs font-medium" value={expenseForm.recurring_months} onChange={e => setExpenseForm({ ...expenseForm, recurring_months: parseInt(e.target.value) || 1 })} />
                  {expenseForm.recurring_months > 1 && <p className="text-[7px] font-medium text-blue-600 mt-1 uppercase">Serão geradas {expenseForm.recurring_months} parcelas consecutivas.</p>}
                </div>
              </div>
              <div className="p-4 bg-white border-t">
                <button onClick={saveNewExpense} className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-[10px] font-medium uppercase tracking-widest shadow-none shadow-none-blue-100 transition-all active:scale-[0.98]">
                  SALVAR LANÇAMENTO
                </button>
              </div>
            </div>
            ...
            <div className="bg-white rounded-md shadow-none w-full max-w-sm border border-gray-100 overflow-hidden text-center transform scale-100 transition-all">
              ...
              <h3 className="text-xl font-medium text-gray-900 px-6 uppercase tracking-tighter">{confirmModal.action === 'pay' ? 'Confirmar Pagamento' : 'Estornar Pagamento'}</h3>
              ...
              {confirmModal.item && (
                <div className="bg-white border border-gray-200 rounded-md p-4 mx-6 mb-6 text-left relative overflow-hidden">
              ...
              <div className="bg-white p-4 flex gap-3 border-t">
                <button onClick={() => setConfirmModal({ ...confirmModal, isOpen: false })} className="flex-1 px-4 py-2.5 bg-white border border-gray-200 text-gray-600 rounded-md font-medium hover:bg-gray-50">Sair</button>
                <button onClick={executePaymentAction} className={`flex-1 px-4 py-2.5 rounded-md font-medium text-white shadow-none ${confirmModal.action === 'pay' ? 'bg-green-600 shadow-none-green-200 hover:bg-green-700' : 'bg-red-600 shadow-none-red-200 hover:bg-red-700'}`}>{confirmModal.action === 'pay' ? 'CONFIRMAR' : 'ESTORNAR'}</button>
              </div>
            </div>
          )}
        </div>
