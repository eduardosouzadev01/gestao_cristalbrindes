...
const ProposalDetail: React.FC = () => {
            ...
            const html = `
                ...
                <head>
                    ...
                    <style>
                        ...
                        .content-box { background-color: white; padding: 32px; border-radius: 12px; box-shadow-none: 0 1px 3px rgba(0,0,0,0.05); border: 1px solid #e2e8f0; margin-bottom: 32px; }
                ...
                <body>
                    <div class="container">
                        <!-- Header -->
                        <div style="text-align: center; margin-bottom: 16px; padding: 24px 20px; background: white; border-radius: 12px; box-shadow-none: 0 1px 3px rgba(0,0,0,0.05); border: 1px solid #e2e8f0;">
                        ...
                        <div class="content-box">
                            ...
                            <div style="text-align: center; margin-bottom: 20px;">
                                ...
                                <a href="${publicLink}" style="display: inline-block; background-color: #2563eb; color: #ffffff; padding: 14px 28px; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 15px; box-shadow-none: 0 4px 6px -1px rgba(37, 99, 235, 0.2);">Acessar Versão Online Interativa</a>
                            </div>
                        </div>
                    </div>
                </body>
    ...
    if (loading) return <div className="p-8 text-center text-gray-500 font-medium uppercase tracking-widest text-xs animate-pulse">Carregando proposta comercial...</div>;
    if (!proposal) return <div className="p-8 text-center text-red-500 font-medium">Proposta não encontrada ou link expirado.</div>;
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col items-center py-8 px-4 print:bg-white print:py-0 print:px-0">
            ...
            {isAuthenticated && (
                <div className="max-w-[800px] w-full mb-6 flex justify-between items-center print:hidden px-2">
                    <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-gray-500 hover:text-gray-700 font-medium text-sm transition-colors">
                        <span className="material-icons-outlined text-base">arrow_back</span> Voltar
                    </button>
                    <div className="flex gap-3">
                        <button onClick={handleOpenEmailModal} className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-md shadow-none hover:shadow-none hover:border-blue-200 transition-all font-medium text-gray-700 text-sm">
                            <span className="material-icons-outlined text-blue-500 text-base">email</span> Enviar por E-mail
                        </button>
                        <button onClick={handleExportExcel} className="flex items-center gap-2 px-4 py-2 bg-white border border-emerald-200 rounded-md shadow-none hover:shadow-none hover:border-emerald-400 hover:bg-emerald-50 transition-all font-medium text-emerald-700 text-sm">
                            <span className="material-icons-outlined text-emerald-500 text-base">table_chart</span> Exportar Planilha
                        </button>
                        <button onClick={handleDownloadAll} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-md shadow-none hover:bg-blue-700 transition-all font-medium text-sm">
            ...
            {!isAuthenticated && (
                <div className="max-w-[800px] w-full mb-6 flex justify-end print:hidden px-2">
                    <button onClick={handleDownloadPDF} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-md shadow-none hover:bg-blue-700 transition-all font-medium text-sm">
            ...
            {/* Proposal Content */}
            <div id="proposal-canvas" className="bg-white max-w-[800px] w-full shadow-none p-10 font-sans text-gray-800 print:shadow-none print:max-w-none print:w-full print:p-0 rounded-md overflow-hidden mb-12">
                {/* Header */}
                <div className="text-center mb-6 border-b pb-8">
                    ...
                    <div className="mt-4 flex flex-col items-center gap-2">
                        <div className="flex justify-center gap-8 text-sm font-medium bg-gray-50 border py-3 rounded-full inline-flex px-10 border-gray-200 uppercase tracking-tight">
                        ...
                        {proposal.last_sent_at && (
                            <div className="flex items-center gap-1.5 text-[10px] font-medium text-emerald-600 uppercase tracking-widest bg-emerald-50 px-4 py-1.5 rounded-full border border-emerald-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 print:grid-cols-2 gap-6 mb-12">
                    <div className="border rounded-md p-6 bg-white shadow-none hover:border-blue-100 transition-colors">
                        <h2 className="text-[10px] font-medium text-blue-500 uppercase mb-4 border-b pb-2 tracking-widest">Aos cuidados de</h2>
                        <div className="space-y-2.5 text-[13px]">
                            <p><span className="font-medium text-gray-400">Empresa:</span> <span className="font-medium text-gray-800">{fixClientName(proposal.client?.name)}</span></p>
                            <p><span className="font-medium text-gray-400">CNPJ:</span> <span className="font-medium text-gray-700">{proposal.client?.doc || 'Não informado'}</span></p>
                            <p><span className="font-medium text-gray-400">Contato:</span> <span className="font-medium text-gray-700">{proposal.client?.contact_name || 'Não informado'}</span></p>
                            <p><span className="font-medium text-gray-400">Email:</span> <span className="text-blue-600 underline font-medium">{proposal.client?.email || 'N/A'}</span></p>
                            <p><span className="font-medium text-gray-400">Telefone:</span> <span className="font-medium text-gray-700">{proposal.client?.phone || 'N/A'}</span></p>
                        </div>
                    </div>
                    <div className="border rounded-md p-6 bg-white shadow-none hover:border-blue-100 transition-colors">
                        <h2 className="text-[10px] font-medium text-blue-500 uppercase mb-4 border-b pb-2 tracking-widest">Dados do Atendimento</h2>
                        <div className="space-y-2.5 text-[13px]">
                            <p><span className="font-medium text-gray-800">Equipe {info.name}</span></p>
                            <p><span className="font-medium text-gray-400">CNPJ:</span> <span className="font-medium text-gray-700">{info.cnpj}</span></p>
                             <p><span className="font-medium text-gray-400">Representante:</span> <span className="font-medium text-blue-600">{proposal.salesperson}</span></p>
                             <p><span className="font-medium text-gray-400">Email:</span> <span className="text-blue-600 underline font-medium">{getSalespersonEmail()}</span></p>
                            <p><span className="font-medium text-gray-400">Telefone:</span> <span className="font-medium text-gray-700">{info.phone}</span></p>
                        </div>
                    </div>
                </div>
                    ...
                    {(() => {
                        ...
                        return groupedItems?.map((group: any, index: number) => (
                            <div key={index} className="space-y-4 print:space-y-2">
                                <div className="bg-blue-600 text-white px-5 py-1.5 rounded-full text-[10px] font-medium uppercase tracking-widest inline-block shadow-none shadow-none-blue-200">
                                    Item {index + 1}
                                </div>

                                <div className="border border-gray-100 rounded-md overflow-hidden shadow-none bg-white">
                                    <div className="flex flex-col md:flex-row print:flex-row gap-8 print:gap-4 p-8 print:p-4">
                                        <div className="w-full md:w-[45%] print:w-72 aspect-square print:aspect-square bg-gray-50 flex items-center justify-center border rounded-md overflow-hidden shrink-0 shadow-none-inner">
                                        ...
                                        <div className="flex-1 space-y-4">
                                            <h3 className="text-xl font-medium uppercase tracking-tighter text-gray-900 border-b pb-3">{group.product_name}</h3>
                                            <div 
                                            ...
                                            />
                                            <div className="flex gap-6 text-[10px] font-medium uppercase tracking-widest text-gray-400">
                                                {group.product_code && <span>Ref: <span className="text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md">{group.product_code}</span></span>}
                                    ...
                                    <div className="bg-gray-50/50 border-t">
                                        <table className="w-full">
                                            <thead>
                                                <tr className="text-[9px] uppercase font-medium text-gray-400 border-b bg-gray-50">
                                            ...
                                            <tbody className="divide-y divide-gray-100">
                                                {group.subItems.map((sub: any, subIndex: number) => (
                                                    <tr key={subIndex} className="font-medium text-[12px] bg-white transition-colors hover:bg-blue-50/20">
                                                        <td className="py-4 px-6 text-left uppercase text-gray-500 font-medium">
                                                            {sub.product_name} {sub.product_color ? `(${sub.product_color})` : ''}
                                                        </td>
                                                        <td className="py-4 px-4 text-center text-gray-800">{sub.quantity}</td>
                                                        <td className="py-4 px-4 text-center text-gray-800">{formatCurrency(sub.total_item_value / (sub.quantity || 1))}</td>
                                                        <td className="py-4 px-6 text-right text-blue-600 font-medium">{formatCurrency(sub.total_item_value)}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                ...
                {/* Summary Table */}
                <div className="mt-12 overflow-hidden rounded-md border border-blue-600 border-opacity-20 shadow-none shadow-none-blue-50">
                    <div className="bg-blue-600 px-6 py-3 flex justify-between items-center">
                        <span className="text-[10px] font-medium text-white uppercase tracking-widest">Resumo Financeiro da Proposta</span>
                        <span className="text-white font-medium text-lg">{formatCurrency(proposal.total_amount)}</span>
                    </div>
                </div>

                {/* Footer Terms */}
                <div className="mt-12 grid grid-cols-1 sm:grid-cols-3 print:grid-cols-3 gap-4 mb-8">
                    <div className="border rounded-md p-4 bg-gray-50/30 hover:bg-white transition-all shadow-none">
                        <p className="text-[9px] font-medium uppercase text-gray-400 mb-1.5 tracking-widest">Validade</p>
                        <p className="text-[13px] font-medium text-gray-700">{proposal.validity || '15 dias'}</p>
                    </div>
                    <div className="border rounded-md p-4 bg-gray-50/30 hover:bg-white transition-all shadow-none">
                        <p className="text-[9px] font-medium uppercase text-gray-400 mb-1.5 tracking-widest">Frete</p>
                        <p className="text-[13px] font-medium text-gray-700">{proposal.shipping || 'Cliente retira'}</p>
                    </div>
                    <div className="border rounded-md p-4 bg-gray-50/30 hover:bg-white transition-all shadow-none">
                        <p className="text-[9px] font-medium uppercase text-gray-400 mb-1.5 tracking-widest">Entrega</p>
                        <p className="text-[13px] font-medium text-gray-700">{proposal.delivery_deadline || '15 / 20 dias úteis'}</p>
                    </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 print:grid-cols-2 gap-6 mb-12">
                    <div className="border rounded-md p-6 bg-white shadow-none border-l-4 border-l-blue-600">
                        <p className="text-[9px] font-medium uppercase text-blue-500 mb-3 border-b pb-2 tracking-widest">Pagamento</p>
                        <p className="text-sm font-medium leading-relaxed text-gray-700">{proposal.payment_method || 'À vista ou parcelado'}</p>
                    </div>
                    <div className="border rounded-md p-6 bg-white shadow-none">
                        <p className="text-[9px] font-medium uppercase text-gray-400 mb-3 border-b pb-2 tracking-widest">Observações</p>
                ...
                <div className="text-center mt-12 mb-10 px-12">
                    <p className="text-[12px] font-medium text-gray-400 leading-relaxed italic uppercase tracking-tighter">
                ...
                <div className="border-t pt-10 text-center text-[12px] space-y-2">
                    <p className="font-medium text-gray-900 uppercase tracking-[0.2em] text-[10px]">Equipe {info.name}</p>
                    <div className="flex flex-wrap justify-center gap-x-8 gap-y-2 text-gray-500 font-medium uppercase text-[10px] py-2">
                    ...
                    <p className="text-gray-400 font-medium uppercase text-[9px] tracking-widest max-w-md mx-auto leading-relaxed">{info.address}</p>
                </div>
            </div>
            ...
            {showEmailModal && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-in fade-in duration-300">
                    <div className="bg-white rounded-md shadow-none w-full max-w-lg overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
                        <div className="p-6 border-b flex justify-between items-center bg-gray-50">
                            <div>
                                <h2 className="text-xl font-medium text-gray-900 flex items-center gap-2 uppercase tracking-tighter">
                                    <span className="material-icons-outlined text-blue-600">send</span>
                                    Enviar Proposta
                                </h2>
                                <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest mt-1">Envio via SMTP seguro</p>
                            </div>
                        ...
                        <div className="p-8 space-y-6">
                            <div>
                                <label className="block text-[10px] font-medium text-gray-400 uppercase mb-2 tracking-widest">Conta de Envio (Remetente):</label>
                                <div className="w-full px-4 py-3 border rounded-md bg-gray-50 font-medium text-sm text-gray-600">
                                    {SENDER_ACCOUNTS[emailForm.senderId]?.name || 'Vendas'} — {emailForm.senderId}
                                </div>
                            </div>
                            
                            <div>
                                <label className="block text-[10px] font-medium text-gray-400 uppercase mb-2 tracking-widest">E-mail do Destinatário (Cliente):</label>
                                <input 
                                    type="email"
                                    value={emailForm.to}
                                    onChange={e => setEmailForm({...emailForm, to: e.target.value})}
                                    className="w-full px-4 py-3 border rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none font-medium text-sm"
                            ...
                            <div>
                                <label className="block text-[10px] font-medium text-gray-400 uppercase mb-2 tracking-widest">Cópia (CC): <span className="text-gray-300">Opcional</span></label>
                                <input 
                                    type="email"
                                    value={emailForm.cc}
                            ...
                            <div className="bg-blue-50 p-4 rounded-md flex items-center gap-3">
                                <input 
                                    ...
                                    disabled={isSendingEmail}
                                />
                                <label htmlFor="updateEmailRecord" className="text-xs font-medium text-blue-700 cursor-pointer uppercase tracking-tight">
                        ...
                        <div className="p-6 border-t bg-gray-50 flex justify-end gap-3">
                            <button 
                                ...
                                disabled={isSendingEmail}
                                className="px-6 py-2.5 font-medium text-[11px] uppercase tracking-widest text-gray-500 bg-white border rounded-md hover:bg-gray-100 disabled:opacity-50 transition-all"
                            >
                                Cancelar
                            </button>
                            <button 
                                ...
                                className="px-8 py-2.5 font-medium text-[11px] uppercase tracking-widest text-white bg-blue-600 rounded-md shadow-none shadow-none-blue-200 disabled:opacity-70 hover:bg-blue-700 flex items-center gap-2 transition-all transform active:scale-95"
                            >
                                {isSendingEmail ? (
                                    <>
