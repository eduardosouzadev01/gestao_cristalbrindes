    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-10 flex items-center">
        <button
          ...
          className="mr-4 p-2 rounded-full bg-white shadow-none hover:bg-gray-50 border border-gray-200 text-gray-500 transition-colors"
        >
          <span className="material-icons-outlined text-xl">arrow_back</span>
        </button>
        <h2 className="text-2xl font-medium leading-7 text-gray-900 sm:text-3xl sm:truncate flex items-center gap-3">
      ...
      {activeStep === 'TYPE' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <button
            ...
            className="group flex flex-col items-center justify-center p-12 bg-white border-2 border-transparent hover:border-blue-500 rounded-md shadow-none hover:shadow-none transition-all duration-300"
          >
            <div className="w-20 h-20 bg-blue-100 text-blue-600 rounded-md flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
              <span className="material-icons-outlined text-4xl">person</span>
            </div>
            <h3 className="text-xl font-medium text-gray-900 mb-2">CLIENTE</h3>
          ...
          <button
            ...
            className="group flex flex-col items-center justify-center p-12 bg-white border-2 border-transparent hover:border-emerald-500 rounded-md shadow-none hover:shadow-none transition-all duration-300"
          >
            <div className="w-20 h-20 bg-emerald-100 text-emerald-600 rounded-md flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
              <span className="material-icons-outlined text-4xl">local_shipping</span>
            </div>
            <h3 className="text-xl font-medium text-gray-900 mb-2">FORNECEDOR</h3>
            <p className="text-sm text-gray-500 text-center">Cadastrar parceiro para fornecimento de produtos ou personalização.</p>
          </button>
        </div>
      ) : (
        <form onSubmit={handleSubmit((data) => savePartnerMutation.mutate(data))} className="bg-white shadow-none rounded-md border border-gray-100 overflow-hidden animate-in zoom-in-95 duration-300">
          <div className="bg-white px-8 py-6 border-b border-gray-100 flex justify-between items-center">
            <div>
              <h3 className="text-lg font-medium text-gray-900 uppercase">Preencha os Dados: {partnerType}</h3>
              <p className="text-xs text-gray-400 font-medium">Todos os campos marcados com * são obrigatórios</p>
            </div>
            <span className={`px-3 py-1 rounded-full text-[10px] font-medium uppercase tracking-wider ${partnerType === 'CLIENTE' ? 'bg-blue-100 text-blue-600' : 'bg-emerald-100 text-emerald-600'}`}>
              {partnerType}
            </span>
          </div>
          ...
          <div className="p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="md:col-span-2">
                <label className="block text-xs font-medium text-gray-400 uppercase mb-2">Nome / Razão Social *</label>
                <input
                  {...register('name')}
                  className={`form-input w-full rounded-md py-3 ${errors.name ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:ring-blue-500 focus:border-blue-500'}`}
              ...
              <div className="md:col-span-2">
                <label className="block text-xs font-medium text-gray-400 uppercase mb-2">Nome do Contato</label>
                <input
                  {...register('contact_name')}
                  className={`form-input w-full rounded-md py-3 ${errors.contact_name ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:ring-blue-500 focus:border-blue-500'}`}
              ...
              <div>
                <label className="block text-xs font-medium text-gray-400 uppercase mb-2">CNPJ / CPF *</label>
                <input
                  {...register('doc')}
                  onChange={(e) => {
                    setValue('doc', formatDoc(e.target.value), { shouldValidate: true });
                  }}
                  className={`form-input w-full rounded-md py-3 ${errors.doc ? 'border-red-500 focus:ring-red-500' : 'border-gray-300'}`}
              ...
              <div>
                <label className="block text-xs font-medium text-gray-400 uppercase mb-2">Telefone *</label>
                <input
                  {...register('phone')}
                  onChange={(e) => {
                    setValue('phone', formatPhone(e.target.value), { shouldValidate: true });
                  }}
                  className={`form-input w-full rounded-md py-3 ${errors.phone ? 'border-red-500 focus:ring-red-500' : 'border-gray-300'}`}
              ...
              <div>
                <label className="block text-xs font-medium text-gray-400 uppercase mb-2">E-mail *</label>
                <input
                  {...register('email')}
                  className={`form-input w-full rounded-md py-3 ${errors.email ? 'border-red-500 focus:ring-red-500' : 'border-gray-300'}`}
              ...
              <div>
                <label className="block text-xs font-medium text-gray-400 uppercase mb-2">E-mail Financeiro</label>
                <input
                  {...register('financial_email')}
                  className={`form-input w-full rounded-md py-3 ${errors.financial_email ? 'border-red-500 focus:ring-red-500' : 'border-gray-300'}`}
              ...
              {partnerType === 'FORNECEDOR' && (
                <div>
                  <label className="block text-xs font-medium text-blue-500 uppercase mb-2">Categoria Fornecedor *</label>
                  <select
                    {...register('supplier_category')}
                    className="form-select w-full rounded-md border-blue-200 py-3 focus:ring-blue-500 focus:border-blue-500"
                  >
              ...
              {partnerType === 'CLIENTE' && (
                <div>
                  <label className="block text-xs font-medium text-purple-500 uppercase mb-2">Vendedor Responsável</label>
                  <select
                    {...register('salesperson')}
                    className="form-select w-full rounded-md border-purple-200 py-3 focus:ring-purple-500 focus:border-purple-500"
                  >
            ...
            <div className="mt-10 flex flex-col sm:flex-row gap-4">
              <button
                ...
                className="flex-1 py-4 bg-blue-500 text-white font-medium rounded-md shadow-none shadow-none-blue-200 hover:bg-blue-600 active:scale-[0.98] transition-all uppercase tracking-widest text-sm disabled:opacity-50 disabled:cursor-not-allowed"
              >
              ...
              <button
                ...
                className="px-8 py-4 bg-gray-100 text-gray-500 font-medium rounded-md hover:bg-gray-200 transition-colors uppercase text-sm"
              >
                VOLTAR
              </button>
            </div>
