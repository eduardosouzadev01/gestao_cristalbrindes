    ...
    <div className="max-w-[1920px] w-full mx-auto px-4 py-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-[10px] font-medium text-gray-400 uppercase tracking-widest">Painel administrativo de pedidos de venda</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-md border border-gray-200 p-2.5 shadow-none">
        <div className="flex flex-wrap gap-2 items-end">
          <div className="flex-1 min-w-[250px]">
            <label className="block text-[8px] font-medium text-gray-400 uppercase tracking-widest mb-0.5">BUSCAR PEDIDO</label>
            <div className="relative">
              ...
              <input
                ...
                className="form-input block w-full pl-14 rounded-md border-gray-300 shadow-none focus:ring-0 focus:border-blue-500 text-xs h-8 font-medium"
                value={searchTerm}
          ...
          <div className="w-40">
            ...
            <select
              ...
              className={`form-select block w-full rounded-md border-gray-300 shadow-none focus:ring-0 focus:border-blue-500 text-[10px] h-8 font-medium py-0 pr-6 ${appUser?.salesperson ? 'bg-gray-50' : ''}`}
          ...
          <div className="w-40">
            ...
            <select
              className="form-select block w-full rounded-md border-gray-300 shadow-none focus:ring-0 focus:border-blue-500 text-[10px] h-8 font-medium py-0 pr-6"
              value={statusFilter}
          ...
          <div className="w-28">
            <label className="block text-[8px] font-medium text-gray-400 uppercase tracking-widest mb-0.5">DATA LIMITE</label>
            <input
              type="date"
              className="form-input block w-full rounded-md border-gray-300 shadow-none focus:ring-0 focus:border-blue-500 text-[10px] h-8 px-2"
              value={limitDateFilter}
              onChange={(e) => setLimitDateFilter(e.target.value)}
            />
          </div>
          <div className="w-28">
            <label className="block text-[8px] font-medium text-gray-400 uppercase tracking-widest mb-0.5">SAÍDA FORN.</label>
            <input
              type="date"
              className="form-input block w-full rounded-md border-gray-300 shadow-none focus:ring-0 focus:border-blue-500 text-[10px] h-8 px-2"
      ...
      <div className="bg-white rounded-md border border-gray-200 shadow-none overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-gray-400 font-medium uppercase text-[10px] tracking-widest">
                  ...
                  filteredOrders.map((order, i) => (
                    ...
                    >
                      <td className="px-3 py-1.5 whitespace-nowrap">
                        <a href={`/pedido/${order.id_original}?mode=view`} onClick={(e) => e.stopPropagation()} className="text-xs font-medium text-blue-600 hover:underline">
                          #{order.id}
                        </a>
                      </td>
                      <td className="px-3 py-1.5 whitespace-nowrap">
                        <span className={`inline-flex items-center px-1.5 py-0.5 rounded-md text-[8px] font-medium uppercase shadow-none ${order.statusColor}`}>
                      ...
                      <td className="px-3 py-1.5">
                        <div className="flex flex-col">
                          <span className="text-xs font-medium text-gray-800 uppercase leading-none truncate max-w-[200px]">{order.client}</span>
                          <span className="text-[8px] text-gray-400 font-medium mt-0.5">{order.cnpj}</span>
                        </div>
                      </td>
                      <td className="px-3 py-1.5 whitespace-nowrap">
                        <span className="text-[10px] font-medium text-gray-600 uppercase">{order.vendedor}</span>
                      </td>
                      <td className="px-3 py-1.5 whitespace-nowrap text-[10px] text-gray-500 font-medium">{order.date}</td>
                      <td className="px-3 py-1.5 whitespace-nowrap">
                        <span className="text-[10px] text-blue-600 font-medium">{order.limitDate ? formatDate(order.limitDate) : '-'}</span>
                      </td>
                      <td className="px-3 py-1.5 whitespace-nowrap text-[10px] text-gray-500 font-medium">{order.supplierDate ? formatDate(order.supplierDate) : '-'}</td>
                      <td className="px-3 py-1.5 whitespace-nowrap text-xs font-medium text-gray-900 text-right">{order.total}</td>
        ...
        <div className="bg-white border-t border-gray-200 px-4 py-2 flex items-center justify-between">
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            ...
            <div>
              <nav className="relative z-0 inline-flex rounded-md shadow-none -space-x-px" aria-label="Pagination">
                <button
                ...
                {[...Array(Math.ceil(totalCount / pageSize))].map((_, i) => (
                  <button
                    key={i}
                    onClick={() => setPage(i)}
                    className={`relative inline-flex items-center px-2.5 py-1 border text-[10px] font-medium ${page === i ? 'z-10 bg-blue-600 border-blue-600 text-white shadow-none' : 'bg-white border-gray-300 text-gray-500 hover:bg-gray-50'}`}
                  >
                    {i + 1}
                  </button>
