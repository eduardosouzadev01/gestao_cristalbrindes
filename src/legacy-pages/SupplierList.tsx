    ...
    <div className="max-w-[1920px] w-full mx-auto px-4 py-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-[10px] font-medium text-gray-400 uppercase tracking-widest">Base de fornecedores, serviços e logística</p>
        </div>
        <Link
          to="/cadastros/novo?tipo=FORNECEDOR"
          className="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-none text-xs font-medium text-white bg-emerald-600 hover:bg-emerald-700 transition-all uppercase tracking-widest"
        >
      ...
      <div className="bg-white rounded-md border border-gray-200 shadow-none overflow-hidden">
        <div className="border-b border-gray-200 bg-white px-3 py-1.5 flex flex-col md:flex-row md:items-center justify-between gap-3">
          <nav className="flex items-center gap-4">
            {[
              ...
              >
                ...
                <span className={`px-1.5 py-0.5 rounded-md text-[8px] font-medium ${activeTab === tab.id ? 'bg-emerald-600 text-white shadow-none' : 'bg-gray-200 text-gray-500'
                  }`}>
          ...
          <div className="relative max-w-xs w-full">
            ...
            <input
              ...
              className="block w-full !pl-10 pr-3 border border-gray-300 focus:ring-0 focus:border-emerald-500 rounded-md text-[10px] h-7 font-medium uppercase placeholder:text-gray-300 tracking-tight shadow-none-inner"
              value={searchTerm}
              ...
              {loading ? (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-[10px] font-medium text-gray-400 uppercase">Sincronizando fornecedores...</p>
                    </div>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-gray-400 italic">
                    ...
                    <p className="text-[10px] font-medium uppercase tracking-widest">Nenhum registro nesta categoria</p>
                  </td>
                </tr>
              ) : (
                currentData.map((c, i) => (
                  <tr key={i} className="hover:bg-emerald-50/30 transition-colors group">
                    <td className="px-4 py-1.5">
                      <div className="flex items-center gap-3">
                        <div className={`h-8 w-8 rounded-md flex-shrink-0 flex items-center justify-center font-medium text-[10px] shadow-none border ${i % 2 === 0 ? 'bg-emerald-600 text-white' : 'bg-gray-800 text-white'}`}>
                          {c.initials}
                        </div>
                        <div className="flex flex-col min-w-0">
                          <span className="text-xs font-medium text-gray-800 uppercase truncate max-w-[280px] leading-tight group-hover:text-emerald-600 transition-colors">
                            {c.name}
                          </span>
                          <span className="text-[8px] font-medium text-gray-400 uppercase tracking-widest mt-0.5">{c.supplier_category}</span>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-1.5 whitespace-nowrap">
                      <span className="text-[10px] font-medium text-emerald-600/70 lowercase hover:underline underline-offset-2">{c.email || '-'}</span>
                    </td>
        ...
        <div className="bg-white border-t border-gray-200 px-4 py-2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <nav className="inline-flex rounded-md shadow-none overflow-hidden" aria-label="Pagination">
              ...
              <button className="z-10 bg-emerald-600 border border-emerald-600 text-white px-2.5 py-1 text-[9px] font-medium">1</button>
              <button className="px-1 py-1 border border-gray-300 bg-white text-gray-400 hover:bg-gray-50 transition-colors">
