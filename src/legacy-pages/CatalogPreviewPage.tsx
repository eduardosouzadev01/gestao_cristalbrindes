    ...
    return (
      <div className="flex items-center justify-center h-[calc(100vh-52px)]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-[10px] font-medium text-gray-300 uppercase tracking-widest">Carregando preview...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex flex-col min-h-[calc(100vh-52px)]" style={{ background: '#d1d5db' }}>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="text-[11px] font-medium text-gray-700 truncate max-w-xs">{catalog.title}</span>
          <span className="text-[9px] text-gray-400 font-medium uppercase tracking-widest">{pages.length} página{pages.length !== 1 ? 's' : ''}</span>
        </div>

        <button
          onClick={handleExportPDF}
          className="inline-flex items-center gap-2 px-4 py-2 rounded-md text-[10px] font-medium uppercase tracking-widest text-white transition-all hover:opacity-90 active:scale-95 shadow-none"
      ...
      <div ref={printRef} className="flex flex-col items-center py-8 gap-8">
        ...
        {catalog.cover_image && (
          <div
            className="pdf-page relative overflow-hidden shadow-none"
            style={{
              width: A4_WIDTH,
              minHeight: A4_HEIGHT,
              background: '#000',
        ...
        {(catalog.back_cover_logo_url || catalog.back_cover_message || catalog.back_cover_website || catalog.back_cover_instagram || catalog.back_cover_phone) && (
          <div
            className="pdf-page relative overflow-hidden shadow-none flex flex-col items-center justify-center p-16 text-center"
            style={{
              width: A4_WIDTH,
              minHeight: A4_HEIGHT,
          ...
          >
            <div className="flex flex-col items-center w-full max-w-2xl space-y-16">
              ...
              {catalog.back_cover_message && (
                <p className="text-3xl font-medium leading-relaxed" style={{ color: catalog.primary_color }}>
                  {catalog.back_cover_message}
                </p>
              )}
