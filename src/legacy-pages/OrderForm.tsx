    ...
    <div className="max-w-[1920px] w-full mx-auto px-4 sm:px-6 lg:px-8 pb-20">
      ...
      <Modal isOpen={transferModal} onClose={() => setTransferModal(false)} title="SOLICITAR TROCA DE VENDEDOR">
        <div className="py-2 space-y-4">
          <div className="bg-orange-50 border border-orange-200 rounded-md p-3 text-sm text-orange-800">
            <p className="font-medium">Pedido: <span className="font-medium">#{orderNumber}</span> | Vendedor atual: <span className="font-medium">{vendedor}</span></p>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-500 uppercase mb-1.5">Transferir para *</label>
            <select
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 uppercase mb-1.5">Motivo da solicitação *</label>
            <textarea
              className="form-textarea w-full rounded-md border-gray-300 text-sm"
              rows={3}
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={() => setTransferModal(false)} className="flex-1 py-2.5 bg-gray-100 text-gray-600 rounded-md text-sm font-medium hover:bg-gray-200 transition-colors">
              Cancelar
            </button>
            <button
              onClick={submitTransferRequest}
              disabled={isTransferring || !transferTargetSalesperson || !transferReason}
              className="flex-1 py-2.5 bg-orange-500 text-white rounded-md text-sm font-medium hover:bg-orange-600 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
            >
      ...
      <Modal isOpen={!!activeModal && activeModal !== 'PRODUTO'} onClose={() => { setActiveModal(null); setNewPartnerData({ name: '', doc: '', phone: '', email: '' }); }} title={`CADASTRAR NOVO ${activeModal}`}>
        <form onSubmit={(e) => { e.preventDefault(); savePartner(); }} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-gray-400 uppercase mb-1">Nome / Descrição <span className="text-red-500">*</span></label>
            <input
              required
          ...
          {activeModal !== 'PRODUTO' && (
            <>
              <div>
                <label className="block text-xs font-medium text-gray-400 uppercase mb-1">CNPJ / CPF <span className="text-red-500">*</span></label>
                <input
                  className="form-input w-full rounded-md border-gray-300"
              ...
              <div>
                <label className="block text-xs font-medium text-gray-400 uppercase mb-1">Telefone <span className="text-red-500">*</span></label>
                <input
                  className="form-input w-full rounded-md border-gray-300"
              ...
              <div>
                <label className="block text-xs font-medium text-gray-400 uppercase mb-1">E-mail <span className="text-red-500">*</span></label>
                <input
                  className="form-input w-full rounded-md border-gray-300"
                  type="email"
          ...
          <button type="submit" className="w-full py-3 bg-blue-500 text-white font-medium rounded-md mt-2 uppercase">SALVAR CADASTRO</button>
        </form>
      </Modal>
      ...
      <Modal isOpen={!!confirmPayModal} onClose={() => setConfirmPayModal(null)} title="CONFIRMAR RECEBIMENTO">
        <div className="text-center py-4">
          ...
          <p className="text-gray-600 mb-6 font-medium">Você confirma que o valor de <span className="font-medium text-gray-900">{confirmPayModal === 'entrada' ? recebimentoEntrada : recebimentoRestante}</span> foi devidamente recebido?</p>
          <div className="flex gap-4">
            <button onClick={() => setConfirmPayModal(null)} className="flex-1 py-3 bg-gray-100 text-gray-500 rounded-md font-medium uppercase text-xs">CANCELAR</button>
            <button onClick={confirmPaymentAction} className="flex-1 py-3 bg-blue-500 text-white rounded-md font-medium uppercase text-xs shadow-none shadow-none-blue-100">CONFIRMAR</button>
          </div>
        </div>
      </Modal>
      ...
      <Modal isOpen={!!confirmCostPaymentModal} onClose={() => setConfirmCostPaymentModal(null)} title="CONFIRMAR PAGAMENTO DE CUSTO">
        <div className="text-center py-4">
          ...
          <p className="text-gray-600 mb-6 font-medium">
            Confirma o pagamento de <span className="font-medium text-gray-900">{confirmCostPaymentModal?.label}</span> no valor de <span className="font-medium text-gray-900">{formatCurrency(confirmCostPaymentModal?.amount || 0)}</span>?
          </p>
          <div className="flex gap-4">
            <button onClick={() => setConfirmCostPaymentModal(null)} className="flex-1 py-3 bg-gray-100 text-gray-500 rounded-md font-medium uppercase text-xs">CANCELAR</button>
            <button onClick={confirmCostPaymentAction} className="flex-1 py-3 bg-blue-500 text-white rounded-md font-medium uppercase text-xs shadow-none shadow-none-blue-100">CONFIRMAR</button>
          </div>
        </div>
      </Modal>
      ...
      <Modal isOpen={!!adjustPriceModal} onClose={() => setAdjustPriceModal(null)} title="AJUSTE MANUAL DE PREÇO">
        <div className="py-4">
          ...
          <div className="mb-4">
            <label className="block text-xs font-medium text-gray-500 uppercase mb-2">Novo Valor {adjustPriceModal?.type === 'unit' ? 'Unitário' : 'Total'}</label>
            <input
              autoFocus
              className="form-input w-full text-center text-2xl font-medium text-blue-600 rounded-md border-blue-300 focus:ring-blue-500"
              value={adjustValue}
          ...
          <div className="mb-6">
            <label className="block text-xs font-medium text-gray-500 uppercase mb-2">Motivo da Alteração <span className="text-red-500">*</span></label>
            <textarea
          ...
          <div className="flex gap-4">
            <button onClick={() => setAdjustPriceModal(null)} className="flex-1 py-3 bg-gray-100 text-gray-500 rounded-md font-medium uppercase text-xs">CANCELAR</button>
            <button onClick={confirmPriceAdjustment} disabled={!adjustComment} className="flex-1 py-3 bg-blue-500 text-white rounded-md font-medium uppercase text-xs shadow-none shadow-none-blue-100 disabled:opacity-50">CONFIRMAR AJUSTE</button>
          </div>
        </div>
      </Modal>
      ...
      <div className="md:flex md:items-center md:justify-between mb-8">
        <div className="flex-1 flex items-center">
          <button onClick={() => navigate(-1)} className="mr-4 p-2 rounded-full bg-white shadow-none border border-gray-200 text-gray-500 hover:bg-gray-50">
            <span className="material-icons-outlined text-xl">arrow_back</span>
          </button>
          <h2 className="text-2xl font-medium text-gray-900 sm:text-3xl flex items-center gap-3 uppercase tracking-tighter">
        ...
        <div className="flex items-center gap-3">
          ...
          {appUser?.salesperson && !isNewOrder && (
            <button
              ...
              className="px-4 py-2 rounded-md border border-orange-200 text-orange-600 bg-orange-50 hover:bg-orange-100 text-xs font-medium transition-all flex items-center gap-1.5"
            >
          ...
          {!isReadOnly && !isNewOrder && (
            <button
              ...
              className="px-4 py-2 rounded-md border border-red-200 text-red-600 bg-red-50 hover:bg-red-100 text-[12px] font-medium transition-all"
            >
              Cancelar Pedido
            </button>
          )}
          {!isReadOnly && (
            <button onClick={validate} disabled={isSaving} className="px-6 py-2.5 rounded-md shadow-none text-[13px] font-medium text-white bg-[#0F6CBD] hover:bg-[#0c5aa5] transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        <div className="mb-6 px-4 py-2 overflow-x-auto">
          <div className="flex items-center justify-center min-w-[700px]">
            {(() => {
              ...
              if (isCancelled) {
                return (
                  <div className="flex items-center gap-3 bg-red-50 border border-red-100 px-8 py-4 rounded-md animate-pulse">
                    <span className="material-icons-outlined text-red-500 text-3xl">cancel</span>
                    <div className="flex flex-col">
                      <span className="text-red-900 font-medium text-xl leading-none uppercase tracking-tighter">Pedido Cancelado</span>
                      <span className="text-red-600 text-[10px] font-medium uppercase tracking-widest mt-1">Este pedido foi descontinuado e não gera mais movimentações</span>
                    </div>
                  </div>
                );
              }
                    ...
                    <div className="flex flex-col items-center flex-shrink-0" style={{ minWidth: 72 }}>
                      <button
                        ...
                        className={`w-11 h-11 rounded-full flex items-center justify-center transition-all duration-200 border-2 ${isCompleted
                          ...
                          : isCurrent
                            ? 'bg-[#0F6CBD] border-[#0F6CBD] text-white shadow-none shadow-none-blue-200/60 scale-110'
                      ...
                      <span className={`text-[9px] font-medium mt-2 text-center uppercase tracking-wider leading-tight ${isCurrent ? 'text-[#0F6CBD]' : isCompleted ? 'text-[#0F6CBD]/70' : 'text-gray-400'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start relative">
        <div className="lg:col-span-8 space-y-6">
          <section className="bg-white shadow-none rounded-md border border-gray-200 p-6">
            <div className="flex items-center gap-2 mb-6 border-b border-gray-100 pb-4">
              <span className="material-icons-outlined text-blue-500">info</span>
              <h3 className="text-xs font-medium text-gray-400 uppercase tracking-wider">Informações Gerais</h3>
            </div>
            <div className="grid grid-cols-12 gap-4">
              <div className="col-span-2">
                <label className="block text-xs font-medium text-blue-500 uppercase mb-2">Pedido <span className="text-red-500">*</span></label>
                <input
                  disabled={isReadOnly || !!location.state?.fromBudget}
                  className={`form-input block w-full text-center rounded-md font-medium py-2 ${errors.includes('orderNumber') ? 'border-red-500 ring-1 ring-red-500' : 'border-blue-500'} ${(isReadOnly || !!location.state?.fromBudget) ? 'bg-gray-100' : ''}`}
              ...
              <div className="col-span-2">
                <label className="block text-xs font-medium text-gray-400 uppercase mb-2">Vendedor <span className="text-red-500">*</span></label>
                <select
                  disabled={isReadOnly || !!appUser?.salesperson}
                  className={`form-select block w-full py-2 rounded-md text-sm font-medium ${errors.includes('vendedor') ? 'border-red-500 ring-1 ring-red-500' : 'border-gray-300'} ${(isReadOnly || !!appUser?.salesperson) ? 'bg-gray-100' : ''}`}
                  value={vendedor}
              ...
              <div className="col-span-3">
                <label className="block text-xs font-medium text-gray-400 uppercase mb-2">Status <span className="text-red-500">*</span></label>
                <div className="relative flex items-center">
                  ...
                  <select
                    ...
                    className={`form-select block w-full pl-10 rounded-md text-sm ${errors.includes('status') ? 'border-red-500 ring-1 ring-red-500' : 'border-gray-300'} ${(isReadOnly && isNewOrder) ? 'bg-gray-100' : ''}`}
                    value={status}
                    onChange={(e) => {
              ...
              <div className="relative col-span-2">
                <label className="block text-xs font-medium text-gray-400 uppercase mb-2">Data do Orç. <span className="text-red-500">*</span></label>
                <div className="flex items-center">
                  <span className="material-icons-outlined absolute left-3 text-gray-400 pointer-events-none">calendar_month</span>
                  {isReadOnly ? (
                    <div className="form-input block w-full pl-10 rounded-md text-sm border-gray-300 bg-gray-100 font-medium text-gray-700 py-2">
                      {formatDate(dataOrcamento)}
                    </div>
                  ) : (
                    <input type="date" lang="pt-BR" className={`form-input block w-full pl-10 rounded-md text-sm ${errors.includes('dataOrcamento') ? 'border-red-500 ring-1 ring-red-500' : 'border-gray-300'}`} value={dataOrcamento} onChange={(e) => setDataOrcamento(e.target.value)} />
                  )}
                </div>
              </div>
              <div className="relative col-span-3">
                <label className="block text-xs font-medium text-gray-400 uppercase mb-2">Data do Pedido <span className="text-red-500">*</span></label>
                <div className="flex items-center">
                  ...
                  {isReadOnly ? (
                    ...
                    <input type="date" lang="pt-BR" className={`form-input block w-full pl-10 rounded-md text-sm ${errors.includes('dataPedido') ? 'border-red-500 ring-1 ring-red-500' : 'border-gray-300'}`} value={dataPedido} onChange={(e) => setDataPedido(e.target.value)} />
                  )}
                </div>
              </div>

              <div className="col-span-12 mt-2 pt-4 border-t border-gray-100">
                <label className="block text-xs font-medium text-gray-400 uppercase mb-2">Emitente</label>
                <div className="grid grid-cols-3 gap-3">
                  {ISSUERS.map(issuer => (
                    <button
                      ...
                      className={`py-2 px-2 border-2 rounded-md font-medium text-[9px] transition-all uppercase ${emitente === issuer.id ? 'border-blue-500 bg-blue-50 text-blue-600' : 'border-gray-100 text-gray-400'} ${isReadOnly ? 'cursor-not-allowed opacity-70' : ''}`}
                    >
                      {issuer.fullName}
                    </button>
                  ))}
          ...
          <section className="bg-white shadow-none rounded-md border border-gray-200 p-6">
            <div className="flex items-center gap-2 mb-6 border-b border-gray-100 pb-4">
              ...
              <h3 className="text-xs font-medium text-gray-400 uppercase tracking-wider">Dados do Cliente</h3>
            </div>
            <div className="grid grid-cols-12 gap-4">
              ...
              <div className="col-span-2">
                <label className="block text-[10px] font-medium text-gray-400 uppercase mb-1">CNPJ / CPF</label>
                <input className="form-input w-full rounded-md border-gray-200 text-sm bg-gray-50" value={clientData.doc} readOnly />
              </div>
              <div className="col-span-2">
                <label className="block text-[10px] font-medium text-gray-400 uppercase mb-1">Telefone</label>
                <input className="form-input w-full rounded-md border-gray-200 text-sm bg-gray-50" value={clientData.phone} readOnly />
              </div>
              <div className="col-span-2">
                <label className="block text-[10px] font-medium text-gray-400 uppercase mb-1">E-mail Contato</label>
                <input className="form-input w-full rounded-md border-gray-200 text-sm bg-gray-50" value={clientData.email} readOnly />
              </div>
              <div className="col-span-2">
                <label className="block text-[10px] font-medium text-gray-400 uppercase mb-1">E-mail Financeiro</label>
                <input disabled={isReadOnly} className={`form-input w-full rounded-md border-gray-200 text-sm ${isReadOnly ? 'bg-gray-100' : ''}`} value={clientData.emailFin} onChange={(e) => setClientData({ ...clientData, emailFin: e.target.value })} />
              </div>
            </div>
          </section>
          ...
          {items.map((item, index) => {
            ...
            return (
              <div key={item.id} id={`item-${item.id}`} className="bg-white rounded-md border border-gray-100 transition-all duration-300 hover:border-blue-200" style={{ boxShadow: 'none' }}>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-medium text-gray-400 uppercase tracking-widest">Produto {index + 1}</span>
                    {isCollapsed && item.productName && (
                      <span className="text-[13px] font-medium text-gray-700 ml-2">{item.productName}</span>
                    )}
                    {isCollapsed && item.productName && (
                  ...
                  <div className="flex items-center gap-3">
                    {isCollapsed && item.productName && (
                      <span className="text-[13px] font-medium text-[#0F6CBD]">{formatCurrency(calculateItemTotal(item))}</span>
                    )}
                    {!isReadOnly && (
                      <button
                  ...
                  <div className="px-5 pb-5">
                    ...
                    <div className="grid grid-cols-12 gap-x-3 gap-y-2 items-end mb-4">
                      ...
                      <div className="col-span-3 lg:col-span-1">
                        <label className="block text-[10px] font-medium text-gray-400 uppercase mb-1">Qtd *</label>
                        <input
                          type="number"
                          disabled={isReadOnly}
                          className={`form-input w-full rounded-md text-center font-medium ${errors.includes(`quantity-${index}`) ? 'border-red-500 ring-1 ring-red-500' : 'border-gray-300'} ${isReadOnly ? 'bg-gray-100' : ''}`}
                          value={item.quantity}
                          onChange={(e) => updateItem(item.id, 'quantity', parseInt(e.target.value) || 0)}
                        />
                      </div>
                      <div className="col-span-6 lg:col-span-2">
                        <label className="block text-[10px] font-medium text-gray-400 uppercase mb-1">Ref *</label>
                        <input
                          type="text"
                          disabled={isReadOnly}
                          className={`form-input w-full rounded-md text-center font-medium border-gray-300 ${isReadOnly ? 'bg-gray-100' : ''}`}
                          value={item.productCode || ''}
                          onChange={(e) => updateItem(item.id, 'productCode', e.target.value)}
                        />
                      </div>
                      <div className="col-span-3 lg:col-span-3">
                        <label className="block text-[10px] font-medium text-gray-400 uppercase mb-1">Cor *</label>
                        {(() => {
                           ...
                           return (
                             <input
                               ...
                               className={`form-input w-full rounded-md border-gray-300 text-[11px] font-medium py-2 ${isReadOnly ? 'bg-gray-100' : ''}`}
                      ...
                      <div className="col-span-6 lg:col-span-2">
                        <label className="block text-[10px] font-medium text-gray-400 uppercase mb-1">Preço Unit.*</label>
                        <input
                          disabled={isReadOnly}
                          className={`form-input w-full rounded-md border-gray-300 text-right font-medium py-2 text-[11px] text-blue-700 bg-blue-50/50 ${errors.includes(`priceUnit-${index}`) ? 'border-red-500 ring-1 ring-red-500' : ''}`}
                          placeholder="R$ 0,00"
                    ...
                    <div className="mb-4">
                      <div className="flex items-center gap-2 mb-1">
                        <label className="text-[9px] font-medium text-gray-400 uppercase">Descrição do Item (Orçamento/Proposta)</label>
                      </div>
                                            <RichTextEditor
                                              className="w-full text-[10px] rounded-md border-gray-200 py-1"
                    ...
                    <div className="space-y-3">
                      <div className="bg-gray-50 p-4 rounded-md grid grid-cols-12 gap-4 items-end border border-gray-100">
                        ...
                        <div className="col-span-6 lg:col-span-3 text-right">
                          <label className="block text-[10px] font-medium text-blue-400 uppercase mb-1">Saída do Fornec. *</label>
                          <input
                            type="date"
                            lang="pt-BR"
                            disabled={isReadOnly}
                            className={`form-input w-full rounded-md text-sm border-gray-300 font-medium text-gray-700 ${errors.includes(`supplierDepartureDate-${index}`) ? 'border-red-500 ring-1 ring-red-500' : ''} ${isReadOnly ? 'bg-gray-100' : ''}`}
                            value={item.supplier_departure_date || ''}
                            onChange={(e) => updateItem(item.id, 'supplier_departure_date', e.target.value)}
                          />
                        </div>
                        <div className="col-span-6 lg:col-span-3 text-right">
                          <label className="block text-[10px] font-medium text-blue-400 uppercase mb-1">Custo Prod.</label>
                          <div className="py-2 font-medium text-blue-600">{formatCurrency(item.quantity * item.priceUnit)}</div>
                        </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                              <div className="col-span-12 md:col-span-5">
                                <div className="flex items-center gap-2">
                                  ...
                                  <label className="block text-[10px] font-medium text-gray-400 uppercase">Valor Real Unitário</label>
                                </div>
                                {item.supplier_payment_date && (
                                  <div className="mt-1 flex items-center gap-1 text-[9px] text-blue-500 font-medium bg-blue-50 px-2 py-0.5 rounded-full w-fit border border-blue-100 uppercase tracking-tighter">
                              ...
                              <div className="col-span-6 md:col-span-3 flex gap-2 items-center">
                                <input
                                  ...
                                  className={`form-input w-full text-sm py-1.5 rounded-md text-right font-medium ${item.priceUnitPaid ? 'bg-green-50 text-green-700 border-green-200' : 'border-gray-300'}`}
                                  placeholder="R$ 0,00"
                                ...
                                <button
                                  ...
                                  disabled={item.priceUnitPaid}
                                  className={`p-2 rounded-md transition-all ${item.priceUnitPaid ? 'bg-green-500 text-white shadow-none' : 'bg-gray-200 text-gray-400 hover:bg-blue-500 hover:text-white'}`}
                              ...
                              <div className="col-span-6 md:col-span-4 pl-3 border-l border-gray-100">
                                <p className="text-[9px] font-medium text-gray-400 uppercase mb-0.5">Custo Total Real</p>
                                <div className="flex items-baseline gap-2">
                                  <span className="text-sm font-medium text-gray-700">{formatCurrency((item.realPriceUnit || 0) * item.quantity)}</span>
                      ...
                      {['custoPersonalizacao', 'layoutCost', 'transpFornecedor', 'transpCliente', 'despesaExtra'].map(f => {
                        ...
                        return (
                          <div key={f} className="grid grid-cols-12 bg-gray-50 p-3 rounded-md border border-gray-100 gap-4 items-center">
                            <div className="col-span-4">
                              <CustomSelect
                            ...
                            <div className="col-span-2">
                              <input
                                disabled={isReadOnly}
                                className={`form-input w-full text-sm border-gray-300 rounded-md text-right font-medium ${isReadOnly ? 'bg-gray-100' : ''}`}
                                placeholder="R$ 0,00"
                            ...
                            {!isNewOrder && (
                              <div className="col-span-6 flex items-center gap-4 border-l border-gray-200 pl-4 w-full">
                                <div className="flex-1">
                                  <label className="block text-[9px] font-medium text-gray-400 uppercase mb-1">Valor Real</label>
                                  <input
                                    disabled={isPaid}
                                    className={`form-input w-full text-xs py-1 rounded-md text-right font-medium ${isPaid ? 'bg-green-50 text-green-700 border-green-200' : 'border-gray-300'}`}
                                    placeholder="R$ 0,00"
                                ...
                                <div className="pt-4">
                                  <button
                                    ...
                                    disabled={isPaid}
                                    className={`p-3 rounded-md transition-all ${isPaid ? 'bg-green-500 text-white shadow-none' : 'bg-gray-200 text-gray-400 hover:bg-blue-500 hover:text-white'}`}
                    ...
                    <div className="mt-6 bg-blue-50/50 rounded-md border border-blue-100 p-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {/* Left: Totals */}
                        <div className="space-y-4">
                          <div className="flex justify-between items-center p-3 bg-white rounded-md border border-gray-200">
                            <span className="text-[10px] font-medium text-gray-500 uppercase">Custo de Cálculo</span>
                    ...
                    <div className="mt-4 flex gap-4 items-center bg-gray-50 rounded-md p-4 border border-gray-200">
                      <div className="flex-[2] border-r border-gray-200 pr-4">
                        <p className="text-[10px] font-medium text-gray-500 uppercase tracking-widest mb-1">Fator de Cálculo</p>
                        <select disabled={isReadOnly} className="form-select bg-white border-gray-300 text-gray-700 font-medium rounded-md w-full text-sm" value={item.fator} onChange={(e) => updateItem(item.id, 'fator', parseFloat(e.target.value))}>
                      ...
                      <div className="flex-1 border-r border-gray-200 pr-4">
                        <p className="text-[10px] font-medium text-gray-500 uppercase tracking-widest mb-1">Saldo Extra (%)</p>
                        <div className="relative">
                          <input
                            type="number"
                            disabled={isReadOnly}
                            className="form-input bg-white border-gray-300 text-gray-700 font-medium rounded-md w-full text-right pr-8 text-sm"
                            placeholder="0%"
                            value={item.extraPct}
                        ...
                        {item.extraPct > 0 && (
                          <div className="mt-1 flex justify-end items-center gap-1">
                            <span className="text-[9px] font-medium text-gray-400 uppercase tracking-wider">Valor Extra:</span>
                      ...
                      <div className="flex-1 border-r border-gray-200 pr-4">
                        <p className="text-[10px] font-medium text-gray-500 uppercase tracking-widest mb-1">BV (%)</p>
                        <div className="relative">
                          <input
                            type="number"
                            disabled={isReadOnly}
                        ...
                        {item.bvPct > 0 && (
                          <div className="mt-1 flex justify-end items-center gap-1">
                            <span className="text-[9px] font-medium text-gray-400 uppercase tracking-wider">Valor BV:</span>
                            <span className="text-[11px] font-medium text-blue-600">
                      ...
                      <div className="flex flex-col gap-1 text-right w-full md:w-auto md:pl-4 mb-2">
                        <div className="flex justify-between items-center gap-4 text-red-500">
                          <span className="text-[10px] font-medium uppercase tracking-wider">Imposto (14%)</span>
                          <span className="text-sm font-medium">{formatCurrency(calculateItemTotal(item) * 0.14)}</span>
                        </div>
                        <div className="flex justify-between items-center gap-4 text-green-600 bg-green-50 px-2 py-0.5 rounded-md border border-green-100">
                          <span className="text-[10px] font-medium uppercase tracking-wider">Margem (Saldo)</span>
                          <span className="text-sm font-medium">{formatCurrency((calculateItemTotal(item) * 0.86) - ((item.quantity * (item.priceUnit || 0)) + (item.custoPersonalizacao || 0) + (item.layoutCost || 0) + (item.transpFornecedor || 0) + (item.transpCliente || 0) + (item.despesaExtra || 0)))}</span>
                        </div>
                        <div
                          onClick={() => openPriceAdjustment(item.id, 'unit', calculateItemTotal(item) / (item.quantity || 1))}
                          className={`cursor-pointer hover:bg-white p-2 rounded-md transition-colors group relative border border-transparent hover:border-blue-100 ${isReadOnly ? 'pointer-events-none' : ''}`}
                        >
                          {!isReadOnly && <span className="material-icons-outlined absolute -top-2 -left-2 text-blue-500 opacity-0 group-hover:opacity-100 bg-white border border-blue-200 rounded-full p-0.5 text-[10px] shadow-none">edit</span>}
                          <div className="flex justify-between items-center text-gray-500 gap-4">
                            <span className="text-[10px] font-medium uppercase tracking-wider">Venda Unitária</span>
                            <span className="text-lg font-medium text-gray-800 decoration-dashed underline underline-offset-4 decoration-gray-300 group-hover:decoration-blue-500">{formatCurrency(calculateItemTotal(item) / (item.quantity || 1))}</span>
                          </div>
                        </div>
                        <div
                          onClick={() => openPriceAdjustment(item.id, 'total', calculateItemTotal(item))}
                          className={`cursor-pointer hover:bg-white p-3 rounded-md transition-colors group relative border border-transparent hover:border-blue-100 ${isReadOnly ? 'pointer-events-none' : ''}`}
                        >
                          {!isReadOnly && <span className="material-icons-outlined absolute -top-2 -left-2 text-blue-500 opacity-0 group-hover:opacity-100 bg-white border border-blue-200 rounded-full p-0.5 text-[10px] shadow-none">edit</span>}
                          <div className="flex justify-between items-end gap-6">
                            <span className="text-[11px] font-medium uppercase text-gray-800">Total Venda do Item</span>
                            <span className="text-3xl font-medium text-blue-600 leading-none decoration-dashed underline underline-offset-4 decoration-blue-200 group-hover:decoration-blue-500">{formatCurrency(calculateItemTotal(item))}</span>
                          </div>
                        </div>
                      </div>
                    </div>
          ...
          {!isReadOnly && (
            <button onClick={addItem} className="w-full py-5 border-2 border-dashed border-blue-200 rounded-md text-blue-400 font-medium text-sm uppercase hover:bg-blue-50 flex items-center justify-center gap-2">
              <span className="material-icons-outlined">add_circle</span> ADICIONAR NOVO ITEM AO PEDIDO
            </button>
          )}

          {!isNewOrder && !appUser?.salesperson && (
            <section className="bg-white shadow-none rounded-md border border-gray-200 p-6 mt-6">
              <div className="flex items-center gap-2 mb-6 border-b border-gray-100 pb-4">
                <span className="material-icons-outlined text-green-500">account_balance_wallet</span>
                <h3 className="text-xs font-medium text-gray-400 uppercase tracking-wider">Gestão Financeira do Pedido</h3>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Card Saldo Estimado */}
                <div className="bg-blue-50 p-4 rounded-md border border-blue-100">
                  <p className="text-[10px] font-medium text-blue-400 uppercase mb-2">Saldo Estimado (1ª Parcela + 2ª Parcela - Custos)</p>
                  <div className="flex justify-between items-end">
                    ...
                    <p className="text-2xl font-medium text-blue-600">{formatCurrency(totalRecebido - items.reduce((acc, item) => acc + (item.quantity * item.priceUnit) + item.custoPersonalizacao + item.transpFornecedor + item.transpCliente + item.despesaExtra + item.layoutCost, 0))}</p>
                  </div>
                </div>

                {/* Card Saldo Real */}
                <div className="bg-green-50 p-4 rounded-md border border-green-100">
                  <p className="text-[10px] font-medium text-green-500 uppercase mb-2">Saldo Real (Recebido - Custos Reais)</p>
                  <div className="flex justify-between items-end">
                    <div>
                    ...
                    <p className={`text-3xl font-medium ${saldoReal < 0 ? 'text-red-500' : 'text-green-600'}`}>{formatCurrency(saldoReal)}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 border-t border-gray-100 pt-4">
                <p className="text-[10px] font-medium text-gray-400 uppercase mb-3">Status das Comissões</p>
                <div className="space-y-3">
                  {entradaConfirmed && (
                    <div className="flex items-center justify-between bg-gray-50 p-3 rounded-md border border-gray-100">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs font-medium text-gray-700 uppercase">Comissão 1ª Parcela</p>
                          <p className="text-[10px] text-gray-400">{formatDate(dataEntrada)}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <span className="font-medium text-blue-600 text-sm">{formatCurrency(parseCurrencyToNumber(recebimentoEntrada) * 0.01)}</span>
                        <span className="px-2 py-1 bg-yellow-100 text-yellow-700 text-[10px] font-medium rounded-full uppercase">Pendente</span>
                      </div>
                    </div>
                  )}
                  {restanteConfirmed && (
                    <div className="flex items-center justify-between bg-gray-50 p-3 rounded-md border border-gray-100">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs font-medium text-gray-700 uppercase">Comissão 2ª Parcela</p>
                          <p className="text-[10px] text-gray-400">{formatDate(dataRestante)}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <span className="font-medium text-blue-600 text-sm">{formatCurrency(parseCurrencyToNumber(recebimentoRestante) * 0.01)}</span>
                        <span className="px-2 py-1 bg-yellow-100 text-yellow-700 text-[10px] font-medium rounded-full uppercase">Pendente</span>
                      </div>
                    </div>
                  )}
        ...
        <div className="lg:col-span-4 relative">
          <div className="space-y-6 sticky top-20">
            ...
            <div className="relative bg-white shadow-none-[0_8px_32px_rgba(0,0,0,0.04)] rounded-md overflow-hidden border border-gray-100">
              ...
              <div className="px-8 py-6 text-center relative z-10 border-b border-gray-50 bg-blue-50/5">
                <p className="text-[10px] font-medium text-blue-600/60 uppercase tracking-[0.2em] mb-2">Total Geral do Pedido</p>
                <h2 className="text-4xl lg:text-4xl font-medium tracking-tight text-[#0F6CBD] mb-4">{formatCurrency(totalPedido)}</h2>
                <div className="inline-flex items-center gap-2 bg-white px-4 py-1.5 rounded-full border border-blue-50 shadow-none">
                  <div className="w-1 h-1 rounded-full bg-blue-500"></div>
                  <p className="text-[10px] font-medium uppercase tracking-widest text-gray-400">
                    Pendente: <span className="text-blue-600 font-medium">{formatCurrency(totalPedido - parseCurrencyToNumber(recebimentoEntrada) - parseCurrencyToNumber(recebimentoRestante))}</span>
                  </p>
                </div>
              </div>
              ...
              <div className="p-6 space-y-6 bg-white rounded-b-[1.5rem] relative z-10">
                <div>
                  <label className="flex items-center gap-1.5 text-[11px] font-medium text-gray-500 uppercase tracking-widest mb-3">
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      <input
                        ...
                        className="form-input w-full bg-white border-gray-200 text-gray-900 rounded-md px-4 py-2.5 font-medium text-lg focus:ring-4 focus:ring-blue-50 focus:border-blue-500 placeholder:text-gray-300 shadow-none transition-all"
                        placeholder="R$ 0,00"
                    ...
                    <div className="relative w-40">
                      {!canEditPayment ? (
                        <div className="flex items-center justify-center w-full bg-gray-50 border border-gray-200 text-gray-600 rounded-md px-3 py-2.5 font-medium text-sm shadow-none h-[46px]">
                        ...
                        <input
                          type="date"
                          lang="pt-BR"
                          className="form-input w-full bg-white border-gray-200 text-gray-700 rounded-md px-3 py-2.5 font-medium text-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-500 shadow-none h-[46px]"
                          value={dataEntrada}
                    ...
                    <button
                      ...
                      className={`h-[46px] w-[46px] rounded-md transition-all flex items-center justify-center shrink-0 border shadow-none ${entradaConfirmed ? 'bg-green-100 border-green-200 text-green-700' : (isReadOnly && !isNewOrder ? 'bg-gray-100 border-gray-200 text-gray-400 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200' : 'bg-white border-gray-200 hover:bg-gray-50 text-gray-500')}`}
                    >
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className={`h-[46px] w-[46px] rounded-md transition-all flex items-center justify-center shrink-0 border shadow-none ${restanteConfirmed ? 'bg-green-100 border-green-200 text-green-700' : (isReadOnly && !isNewOrder ? 'bg-gray-100 border-gray-200 text-gray-400 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200' : 'bg-white border-gray-200 hover:bg-gray-50 text-gray-500')}`}
                    >
            ...
            <div className="bg-white shadow-none rounded-md p-6 border border-gray-200">
              <div className="flex items-center gap-2 mb-4 text-blue-500">
                <span className="material-icons-outlined">sticky_note_2</span>
                <h3 className="text-xs font-medium uppercase tracking-wider text-gray-500">Observações</h3>
              </div>
              <textarea
                disabled={isReadOnly}
                className={`form-textarea w-full rounded-md border-gray-200 text-xs min-h-[100px] ${isReadOnly ? 'bg-gray-50' : ''}`}
            ...
            <div className="bg-white/90 backdrop-blur-sm rounded-md border border-gray-200/80 p-5" style={{ boxShadow: 'none' }}>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <span className="material-icons-outlined text-[#0F6CBD] text-[18px]">history_toggle_off</span>
                  <h3 className="text-[11px] font-medium uppercase tracking-widest text-gray-500">Histórico</h3>
                </div>
              </div>
                        ...
                        return (
                          <div key={i} className={`flex items-start gap-2 relative ${isEven ? 'flex-row-reverse' : 'flex-row'}`}>
                            ...
                            <div className={`flex-1 max-w-[45%] ${isEven ? 'text-right' : 'text-left'}`}>
                              <div className={`inline-block bg-white border border-gray-100 rounded-md px-3 py-2.5 shadow-none text-left w-full`}>
                                <div className="flex items-center justify-between gap-2 mb-1">
                                  <span className={`text-[9px] font-medium uppercase tracking-wide truncate ${isCristal ? 'text-emerald-600' : 'text-[#0F6CBD]'
                                    }`}>
                                    {l.user.split('@')[0]}
                                  </span>
                            ...
                            <div className="flex-shrink-0 flex items-start justify-center" style={{ width: 16, paddingTop: 10 }}>
                              <div className={`w-3 h-3 rounded-full border-2 border-white shadow-none ${dotColor}`} />
                            </div>
                            {/* Spacer */}
              ...
              <div className="pt-3 border-t border-gray-100 flex gap-2">
                <input
                  className="form-input flex-1 rounded-md border-gray-200 text-xs"
                ...
                <button
                  ...
                  disabled={!historia}
                  className="p-2 bg-[#0F6CBD] text-white rounded-md disabled:opacity-40 hover:bg-[#0c5aa5] transition-colors"
                >
                  <span className="material-icons-outlined text-sm">send</span>
                </button>
              </div>
            ...
            <div className="bg-white shadow-none rounded-md p-6 border border-gray-200">
              <div className="flex items-center gap-2 mb-6 text-blue-500">
                ...
                <h3 className="text-xs font-medium uppercase tracking-wider text-gray-500">Comercial</h3>
              </div>
              <div className="grid grid-cols-12 gap-4">
                <div className="col-span-4">
                  <label className="block text-[10px] font-medium text-gray-400 uppercase mb-1">Faturamento *</label>
                  <select
                    disabled={isReadOnly}
                    className={`form-select w-full rounded-md text-sm ${errors.includes('modalidade') ? 'border-red-500 ring-1 ring-red-500' : 'border-gray-200'} ${isReadOnly ? 'bg-gray-100' : ''}`}
                    value={modalidade}
                ...
                <div className="col-span-4">
                  ...
                  <label className="block text-[10px] font-medium text-gray-400 uppercase mb-1">Limite Receb.</label>
                  {isReadOnly ? (
                    <div className="form-input w-full rounded-md text-sm border-gray-200 bg-gray-100 font-medium text-gray-700 py-2">
                    ...
                    <input
                      type="date"
                      lang="pt-BR"
                      className={`form-input w-full rounded-md text-sm ${errors.includes('dataLimite') ? 'border-red-500 ring-1 ring-red-500' : 'border-gray-200'}`}
                      value={dataLimite}
                ...
                <div className="col-span-4">
                  <label className="block text-[10px] font-medium text-gray-400 uppercase mb-1">Nº NF</label>
                  <input
                    disabled={isReadOnly}
                    className={`form-input w-full rounded-md border-gray-200 text-sm ${isReadOnly ? 'bg-gray-100' : ''}`}
                ...
                <div className="col-span-4">
                  <label className="block text-[10px] font-medium text-gray-400 uppercase mb-1">Req. de Compra</label>
                  <input
                    disabled={isReadOnly}
                    className={`form-input w-full rounded-md border-gray-200 text-sm ${isReadOnly ? 'bg-gray-100' : ''}`}
                ...
                <div className="col-span-4">
                  <label className="block text-[10px] font-medium text-gray-400 uppercase mb-1">LAYOUT</label>
                  <input
                    disabled={isReadOnly}
                    className={`form-input w-full rounded-md border-gray-200 text-sm ${isReadOnly ? 'bg-gray-100' : ''}`}
            ...
            <div className="bg-white shadow-none rounded-md p-6 border border-gray-200">
              <div className="flex items-center gap-2 mb-6 text-blue-500">
                ...
                <h3 className="text-xs font-medium uppercase tracking-wider text-gray-500">Extrato Consolidado</h3>
              </div>
              <div className="space-y-3 mb-6">
                {items.map((item, i) => (
                  <div key={item.id} className="flex justify-between items-start border-b border-gray-50 pb-2">
                    <div className="flex gap-3">
                      <span className="text-gray-200 font-medium text-sm">{i + 1}</span>
                      <div className="w-32 overflow-hidden">
                        <p className="font-medium text-gray-900 text-[10px] uppercase truncate">{item.productName || 'Item em Definição'}</p>
                        <p className="text-[10px] text-gray-400">{item.quantity} UN @ {formatCurrency(item.priceUnit)}</p>
                      </div>
                    </div>
                    <span className="font-medium text-gray-900 text-xs">{formatCurrency(calculateItemTotal(item))}</span>
                  </div>
                ))}
              </div>
              <div className="pt-2 border-t-2 border-blue-50">
                <p className="text-[10px] font-medium text-gray-400 uppercase mb-1">Total Pedido</p>
                <p className="text-4xl font-medium text-blue-500">{formatCurrency(totalPedido)}</p>
              </div>
            </div>
          </div>
        </div>
