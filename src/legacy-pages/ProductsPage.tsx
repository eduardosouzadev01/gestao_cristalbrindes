        ...
        <div className="max-w-[1920px] w-full mx-auto px-4 py-4 space-y-4">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-blue-600 rounded-md flex items-center justify-center shadow-none">
                        <span className="material-icons-outlined text-white text-lg">inventory_2</span>
                    </div>
                    <div>
                        <h1 className="text-xl font-medium leading-none text-gray-900 uppercase tracking-tighter">CATÁLOGO DE PRODUTOS</h1>
                        <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest mt-0.5">Gestão centralizada de itens e SKUs</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className="inline-flex items-center px-3 py-1.5 bg-white border border-gray-300 text-[10px] font-medium uppercase tracking-widest text-gray-700 rounded-md shadow-none hover:bg-gray-50 transition-all gap-2 cursor-pointer"
                    >
                    ...
                    <button
                        onClick={handleSync}
                        disabled={syncing}
                        className={`inline-flex items-center px-3 py-1.5 border border-transparent rounded-md shadow-none text-[10px] font-medium uppercase tracking-widest text-white ${syncing ? 'bg-blue-400' : 'bg-blue-600 hover:bg-blue-700'} transition-all gap-2 cursor-pointer`}
                    >
            ...
            <div className="grid grid-cols-12 gap-3 items-start">
                {/* Search and Source */}
                <div className="col-span-12 lg:col-span-7 bg-white rounded-md border border-gray-200 p-2.5 shadow-none flex items-end gap-3">
                    <div className="flex-1">
                        <label className="block text-[8px] font-medium text-gray-400 uppercase tracking-widest mb-1">BUSCAR PRODUTO</label>
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-14 pr-4 border border-gray-300 rounded-md h-8 text-xs font-medium uppercase placeholder:text-gray-300 tracking-tight shadow-none-inner"
                                value={searchTerm}
                    ...
                    <div className="w-40">
                        <label className="block text-[8px] font-medium text-gray-400 uppercase tracking-widest mb-1">FORNECEDOR</label>
                        <select
                ...
                <div className="col-span-12 lg:col-span-5 grid grid-cols-3 gap-2">
                    {[
                        ...
                        <div key={stat.label} className="bg-white rounded-md border border-gray-200 p-2 shadow-none flex items-center justify-between group">
                            <div className="min-w-0">
                                <p className="text-[7px] font-medium text-gray-400 uppercase tracking-widest leading-none mb-1">{stat.label}</p>
                                <p className="text-sm font-medium text-gray-900 leading-none">{stat.value}</p>
                            </div>
                            <div className={`w-6 h-6 bg-${stat.color}-50 rounded-md flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform`}>
            ...
            <div className="bg-white rounded-md border border-gray-200 shadow-none overflow-hidden">
                            ...
                            {loading ? (
                                <tr>
                                    <td colSpan={5} className="px-6 py-12 text-center pointer-events-none">
                                        <div className="flex flex-col items-center gap-2">
                                            ...
                                            <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest">Sincronizando catálogo...</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : products.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="px-6 py-12 text-center">
                                        <p className="text-[10px] font-medium text-gray-300 uppercase italic">Nenhum produto localizado</p>
                                    </td>
                                </tr>
                            ) : (
                                products.map((product) => (
                                    <tr key={product.id} className="hover:bg-blue-50/30 transition-colors group">
                                        <td className="px-4 py-1.5">
                                            <div className="flex items-center gap-3">
                                                <div className="w-8 h-8 bg-gray-100 rounded-md border border-gray-200 overflow-hidden flex items-center justify-center flex-shrink-0">
                                                ...
                                                <div className="flex flex-col min-w-0">
                                                    <span className="text-xs font-medium text-gray-800 uppercase truncate max-w-[400px] leading-tight group-hover:text-blue-600 transition-colors">
                                                        {product.name}
                                                    </span>
                                                    <span className="text-[8px] font-medium text-gray-400 uppercase tracking-widest mt-0.5">REF: {(product as any).code || '---'}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-4 py-1.5 whitespace-nowrap">
                                            <span className={`px-1.5 py-0.5 rounded-md text-[8px] font-medium uppercase shadow-none border ${(product as any).source === 'XBZ' ? 'bg-orange-50 text-orange-600 border-orange-100' :
                                        ...
                                        <td className="px-4 py-1.5 whitespace-nowrap">
                                            <div className="flex items-center gap-2">
                                                <div className={`w-1.5 h-1.5 rounded-full shadow-none ${((product as any).stock || 0) > 0 ? 'bg-emerald-500' : 'bg-red-500'}`}></div>
                                                <span className={`text-[10px] font-medium ${((product as any).stock || 0) > 0 ? 'text-gray-700' : 'text-red-600'}`}>
                                        ...
                                        <td className="px-4 py-1.5 text-right whitespace-nowrap">
                                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button
                                                    ...
                                                    className="p-1 px-2 text-[9px] font-medium uppercase tracking-widest text-blue-600 hover:bg-blue-100 rounded-md transition-colors cursor-pointer"
                                                >
                                                ...
                                                <button
                                                    ...
                                                    className="p-1 px-2 text-[9px] font-medium uppercase tracking-widest text-red-600 hover:bg-red-100 rounded-md transition-colors cursor-pointer"
                                                >
                                                    EXCLUIR
                                                </button>
                                            </div>
                ...
                {!loading && totalCount > 0 && (
                    <div className="bg-white border-t border-gray-200 px-4 py-2 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            <nav className="inline-flex rounded-md shadow-none overflow-hidden" aria-label="Pagination">
                                <button
                                ...
                                <div className="flex items-center">
                                    {(() => {
                                        ...
                                        return getPageNumbers().map((pageNum, i) => (
                                            pageNum === '...' ? (
                                                <span key={`dots-${i}`} className="px-1.5 py-1 border border-gray-300 bg-white text-[9px] text-gray-400">...</span>
                                            ) : (
                                                <button
                                                    key={pageNum}
                                                    onClick={() => setPage(Number(pageNum))}
                                                    className={`px-2.5 py-1 border border-gray-300 text-[9px] font-medium transition-all ${page === Number(pageNum)
