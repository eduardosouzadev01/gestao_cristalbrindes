...
const SlotEditorModal: React.FC<SlotEditorModalProps> = ({ slot, onClose, onSave }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-md shadow-none overflow-hidden"
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <h3 className="text-sm font-medium text-gray-900 uppercase tracking-tight">Editar Slot</h3>
            <p className="text-[10px] text-gray-400 mt-0.5">
              Produto: {slot.product?.name || 'Sem produto'} · {slot.orientation === 'vertical' ? 'Vertical' : 'Horizontal'}
            </p>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-md flex items-center justify-center hover:bg-gray-100 transition-colors">
        ...
        <div className="overflow-y-auto p-6 space-y-5">
          {/* Image */}
          <div>
            <label className="block text-[9px] font-medium text-gray-400 uppercase tracking-widest mb-2">Imagem Exclusiva</label>
            <div className="flex gap-3 items-start">
              ...
              <div className="flex-1 space-y-2">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 border border-gray-200 rounded-md text-[10px] font-medium text-gray-500 hover:bg-gray-50 transition-all disabled:opacity-50"
                >
          ...
          {/* Title */}
          <div>
            <label className="block text-[9px] font-medium text-gray-400 uppercase tracking-widest mb-1.5">
          ...
          {/* Description */}
          <div>
            <label className="block text-[9px] font-medium text-gray-400 uppercase tracking-widest mb-1.5">Descrição Personalizada</label>
            <textarea
              value={customDesc}
              onChange={e => setCustomDesc(e.target.value)}
              placeholder={slot.product?.description || 'Descrição do produto...'}
              rows={3}
              className="w-full px-3 py-2 border border-gray-200 rounded-md text-xs resize-none focus:outline-none focus:ring-2 focus:ring-blue-200"
            />
          </div>
          ...
          <div className="flex items-center gap-4">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              >
                <div
                  className="w-4 h-4 bg-white rounded-full shadow-none absolute top-0.5 transition-all"
                  style={{ left: showCode ? '17px' : '2px' }}
                />
              </div>
              <span className="text-[10px] font-medium text-gray-600 uppercase tracking-widest">Mostrar código</span>
            </label>
            ...
            {slot.product?.unit_price && (
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-[10px] font-medium text-gray-600 uppercase tracking-widest">Mostrar preço</span>
              </label>
            )}
          </div>
        </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 border border-gray-200 rounded-md text-[10px] font-medium uppercase tracking-widest text-gray-500 hover:bg-gray-50 transition-all"
          >
            Cancelar
          </button>
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex-1 py-2.5 rounded-md text-[10px] font-medium uppercase tracking-widest text-white transition-all disabled:opacity-60"
