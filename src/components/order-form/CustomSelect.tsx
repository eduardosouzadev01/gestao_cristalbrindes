    ...
    return (
        <div className="relative" ref={containerRef}>
            ...
            <div
                ...
                className={`form-input w-full rounded-md flex justify-between items-center cursor-pointer bg-white py-2 ${error ? 'border-red-500 ring-1 ring-red-500' : 'border-gray-300'} ${disabled ? 'bg-gray-100 cursor-not-allowed' : ''}`}
            >
            ...
            {isOpen && !disabled && (
                <div className="absolute z-[110] w-full mt-1 bg-white border border-gray-200 rounded-md shadow-none overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    <div className="max-h-48 overflow-y-auto">
                            ...
                            if (hasCategories && filtered.length > 0) {
                                ...
                                const renderedItems = categories.map(cat => (
                                    <div key={cat as string}>
                                        <div className="px-4 py-1.5 text-[10px] font-medium text-blue-600 bg-blue-50 uppercase tracking-widest border-y border-blue-100">{cat === 'GRAVACOES' ? 'PERSONALIZAÇÃO' : (cat as string)}</div>
                                ...
                                if (noCategory.length > 0) {
                                    renderedItems.push(
                                        <div key="no-category">
                                            <div className="px-4 py-1.5 text-[10px] font-medium text-gray-400 bg-gray-50 uppercase tracking-widest border-y border-gray-100">OUTROS</div>
                                            {noCategory.map(opt => (
                                                <div key={opt.id} className="px-4 py-2 text-sm hover:bg-blue-50 cursor-pointer text-gray-700 flex justify-between group" onClick={() => { onSelect(opt); setSearch(opt.name); setIsOpen(false); }}>
                                                    <span>{opt.name}</span>
                                                    {opt.code && <span className="text-gray-400 text-xs font-mono group-hover:text-blue-500">{opt.code}</span>}
                        ...
                        <div
                            className="px-4 py-2 text-sm text-blue-600 font-medium hover:bg-blue-50 cursor-pointer border-t border-gray-100 flex items-center gap-2"
