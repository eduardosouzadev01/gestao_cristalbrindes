...
export const useOrderItems = (initialItems: any[] = [], referenceNumber?: string) => {
    ...
    const totalBV = itemsWithContext.reduce((acc, item) => acc + (calculateItemTotal(item) * ((item.bvPct || 0) / 100)), 0);
    const totalTaxes = itemsWithContext.reduce((acc, item) => acc + (calculateItemTotal(item) * ((item.mockNF ?? 14) / 100)), 0);
    const totalProfit = totalRevenue - totalCostsReal;
